...
| BUG-028 | Low→**High** | Fixed — ported and integrated | pending port+land | RE-VERIFIED (real red/green) | Severity raised: the cleanup `DELETE ... WHERE batch_id = ANY($2) AND animal_count = 0` deletes sibling arms already at zero, AND `vaccination_drive_assignment_members` has `ON DELETE CASCADE` on `assignment_id` — so it silently destroys those arms' per-goat ledger too. Fixed by `RETURNING vda.assignment_id` from both decrement UPDATEs and keying the DELETE on those exact ids. RED: `already-zero sibling arm rows = 0, want 1`. +5 adversarial dimension tests |
| BUG-029 | Medium | **Closed — resolved by design (MAINTAINER DECISION 2026-07-24)** | n/a | RE-VERIFIED (code read) | Original premise (membership "CHOSEN at persist time in `goat_id` order") is **stale**. Persist-time binding `syncVaccinationDriveAssignmentMembersTx` (`visit_shot_lock.go:326`) is **deterministic, partition-aware, and provable**, not a blind `goat_id` slice: each goat binds to the arm covering its **own physical shed partition** (`LEFT JOIN goat_shed_partitions`, PK-unique, no fan-out — the BUG-035 fix), the vaccine **lane** is part of the cell key so a goat's ET+TT obligation can never bind to the PPR arm, and `goat_id` rank is only the **last-resort tiebreak** inside a lane. Replay-safe (prune+reinsert converges) and set-based (scale-guard clean). **Ownership split ratified:** the planner (`OperatorDrivePlanner`, `operator_drive_planner.go`) owns count/cap/date/operator split; the persist SQL owns exact per-goat member binding. Both are deterministic functions of the same inputs. **Invariant:** `count(DISTINCT goat_id)` per assignment == that row's `animal_count` (except when declared counts and the real obligation set disagree — membership is total, never invented) — proven by `TestAssignmentBindingOneToManyPageBoundaryDateShiftParkScopeHierarchyStatusMatrixParity` (`processintegrity/adapters/postgres/assignment_binding_parity_test.go`, exercises 1:N fan-out, page boundary, date shift, park-scope hierarchy, status matrix, both exact + legacy arms). The planner's dormant `GoatIDs` plumbing (`canonicalGoatIDs`/`sliceGoatIDs`/`DrivePlanAssignment.GoatIDs`) stays as a count-only split aid; **no full planner-owns-membership refactor** unless a real divergence case surfaces (it would replace partition-aware binding with count-order naming — strictly less correct without partition data, and adds a per-partition read on the sweep hot path). |
...
| BUG-032 | High | Partly fixed — calendar 18→0, 40 remain | ported and integrated | RE-VERIFIED (calendar only) | True baseline on clean `f97e7fe6` is **58 failures across 8 packages**, not ~24. Calendar's 18 fixed test-only, 4 root causes: (RC-1, 13 tests) query window built from a CLOCK INSTANT not a business day — a batched park drive anchors at 00:00 IST so `dueAt.Add(-1h)` fell outside the window; these only passed during a ~1h slice of each day. (RC-2, 10) `drive_summary` became opt-in at `c7c9d74a`, tests never updated. (RC-3, 2) "overdue" fixtures at `now−2h` only cross the business-day boundary between 00:00–02:00 IST. (RC-4) `""` bound into `$3::date`. Remaining 40 split into BUG-036/037/038 + stale-test work |
| BUG-036 | High | Fixed — ported and integrated | pending port | RE-VERIFIED (EXPLAIN before/after) | Both traced to commit `b5a21b1a` replacing a SARGable bound with a predicate over LEFT JOIN/LATERAL output: `oi.due_at <= $3` became `COALESCE(vda.assignment_planned_at, ob.planned_date…, oi.due_at) <= $3`. `vda`/`ob` are JOIN outputs, so no index on `obligation_instances` is usable and every tenant row materialises first. The 500k gate (`b73e31d5`) PREDATES that commit, so its ceiling was calibrated on the pre-regression plan. Fix: index-usable superset CTEs as a **UNION of two index arms** (the `OR … IN (…)` form was tried first and fell back to Seq Scan at cost 2,129,542), LATERAL pre-aggregated via `DISTINCT ON` reproducing the `ORDER BY … LIMIT 1` tie-break exactly, and the authoritative `COALESCE(...) <= $3` kept unchanged as residual filter. No new index needed. **(a) 500,001 rows → 1,440. (b) 2505.3ms → 13.7ms.** +2 grain-parity tests running the retired LATERAL and the new CTE side by side. ORIGINAL: **Two genuine scale regressions**, both banned compute-on-read under `docs/decisions/scale-anti-patterns.md`. (a) `vaccinationOperationsSQL @500k: obligation_instances scan touched 500001 rows (> ceiling 50000) — lost due-window selectivity`. (b) `processIntegrityCanonicalCountsSQL @500k: obligation_instances hit a "Seq Scan"; rootCost=3239202 execTime=2505.3ms`. These are product defects the plan tests correctly caught, NOT fixture drift |
...
| BUG-040 | **P1 — production-real, proven by CPT reseed** | Fixed | pending land | RE-VERIFIED (CPT reseed real DB: 14→0, 600→0) | **Corrects the BUG-029/BUG-035 "partition-aware and provable" claim — it was a silent no-op for numeric-partition sheds.** The BUG-035 fix added `LEFT JOIN goat_shed_partitions` + `ORDER BY … s.partition_label = gsp.partition_label DESC` to bind each goat to its own partition arm. But the two writers normalize the label differently: `goat_shed_partitions.partition_label` = normalized form (`seed-vaccination-real.normalizeSeedShedPartition`: "Gandhi 1"→`1`), while `vaccination_drive_assignments.partition_label` = display form (`Part 1`). For numeric-partition sheds (Gandhi 1/2/3, Old Yashoda 5) the equality `s.partition_label = gsp.partition_label` is NEVER true, so the partition-match tiebreak is inert and `DISTINCT ON` falls to alphabetical `s.partition_label` — collapsing every partition of a shed onto its first arm. **Proven on the CPT reseed (origin/main a99119a0, isolated DB :55448):** Gandhi/Part1/2026-07-25 `animal_count=42` but **84 members**; sibling Part3 `42` sized / **0 members**; 14 assignment rows with `animal_count != count(distinct member goat)`; 600 members partition-misbound. Godel/Mandela ("Part N" on both sides) matched by luck and hid it. The exit/death path (`repository.go:3875-3876`, the BUG-013 partition narrowing) shares the same equality, so a Gandhi-style goat exit failed to decrement its own arm. Fix: canonicalize both sides — `regexp_replace(lower(btrim(x)),'^part[[:space:]]+','')` — at the membership tiebreak (`visit_shot_lock.go:396`) and the exit filter (`repository.go:3876`). RE-VERIFIED on real reseed: B1 mismatch **14→0**, partition-misbound **600→0**, Gandhi/07-25 now Part1 42/42 + Part3 42/42, BUG-035 vaccine-lane wrong-bindings stay **0**, `check-expected-drive-schedules: DB matches` (cap=200 N=1). Adversarial Gandhi-1/2/3 multi-partition test still owed (landed BUG-035 tests use matching/single-partition labels, so they stay green over this bug). |
| BUG-041 | High | **Fixed — root cause was the combo-align MERGE, not the planner** | pending land | RE-VERIFIED (two throwaway-Postgres regression tests: bug reproduced then closed, both shapes, idempotent) | **Correction to the original diagnosis:** the drop is NOT the planner failing to emit rows. `driveAssignmentsForParkConsolidation`/`driveAssignmentsForUnbatched` DO bucket every (shed, partition) and union the lane. The real cause is `mergeUnfinalizedBatchIntoPlannedDate` (`repository.go`): when combo-align moves a source batch's obligations into a compatible target batch (`UpdateBatchPlannedDate` → 23505 → merge), it moved `obligation_instances.batch_id` and superseded the source but **never rebuilt the target's `vaccination_drive_assignments`**. The target then held more obligations than its drive rows knew about — exactly the two shapes: an Old-Yashoda shed merged into a Gandhi-only target (no OY arm), and an FMD obligation merged into an HS-only target (no FMD lane). Fix: the merge now (a) DELETEs the stale source drive rows (member rows cascade, freeing the moved obligations from `members UNIQUE(tenant_id,obligation_id)`) and RETURNS the target batch id; (b) `AlignComboDrives` then rebuilds that target through the real planner — `DriveRebuildInputsForBatch` reads the FULL attached goat-obligation set, `driveAssignmentsForUnbatched` buckets by (park, shed, physical_shed, partition, vaccine_rule_ids), `distributeVaccinationDriveAssignmentsExcludingBatch` distributes across real operators using a capacity read that excludes the target's own load (`AvailableVaccinationOperatorsForDriveExcludingBatch`, session cache bypassed), and `ReplaceVaccinationDriveAssignmentsForBatch` does an atomic delete+insert+membership-sync. No NULL-operator rows are ever written: if no executable operator exists the rebuild is SKIPPED and recorded on the session as `no_operator` (observable, not silent). Preserves BUG-035 lane binding, BUG-039 sizing, BUG-040 partition canonicalization. Regression proof: `bug_041_merged_batch_drive_rebuild_integration_test.go` (cross-shed BT + sibling-lane FMD/HS): each asserts pre-rebuild unbound > 0, then post-rebuild 0 unbound, no obligation in >1 member, rule ∈ vaccine_rule_ids, no NULL operators, animal_count == distinct member goats, target planned_date on every row, operator/day ≤ 200, source batch 0 drive rows, and idempotent rerun.<br/>_(superseded diagnosis:)_ Planner assignment-coverage gap, distinct from BUG-040 (membership SQL). After a clean CPT reseed, **9 planned goat obligations sit on assignment-bearing batches with no membership row** — they are due but attached to no operator drive lane. Two shapes: (a) **8 Blue-Tongue goats** physically in `Old Yashoda/1` land in a 2026-08-08 BT batch whose only assignment rows are `Gandhi/Part 1` + `Gandhi/Part 3` — no Old-Yashoda arm exists (cross-shed park-batch, planner emitted no assignment row for the goat's shed); (b) **1 FMD goat** (`ec5e4e75…`, `Godel 1/Part 1`, 2026-09-07) whose batch materialized only the sibling `hs_adult_w1` lane — no FMD assignment row, so its FMD obligation can't bind. Root is in the app-layer drive planner (`OperatorDrivePlanner`, `operator_drive_planner.go`) not emitting an assignment row for every (shed, vaccine-lane) that has due obligations in the batch. Affects only non-ET+TT/PPR tail (BT/FMD) here; ET+TT 324 + PPR 231 bind 100%. Not fixed by the BUG-040 change. RFIDs/goat_ids captured in `context/repo-audits/` reseed run. **Blocks calling the stg seed clean.** |

**STG SEED NOTE (2026-07-24, before seeding Google `goatos-stg`):** the CPT reseed on origin/main surfaced two partition-grain defects that a green build hid. Before/while seeding stg: (1) BUG-040 is fixed — verify partition_label is canonicalized across the drive planner writer AND `goat_shed_partitions` on stg data too (any shed whose source label is numeric, e.g. "Gandhi 1/2/3", "Old Yashoda 5", is the exposed case; "X - Part N" sheds are not); after stg seed assert `SELECT count(*) FROM vaccination_drive_assignments a LEFT JOIN … WHERE a.animal_count <> count(distinct member goat) = 0` and 0 partition-misbound members. (2) BUG-041 is still OPEN — after stg seed, run the A1 unbound-obligation proof (`planned obligation on an assignment-bearing batch with no membership row = 0`); if non-zero, the planner left due animals off every operator's drive sheet. Do not certify the stg seed as clean until both proofs are zero. Root fixes belong in the seed/planner path, not in a post-hoc stg data patch.

**Current working state (2026-07-24, after landing the vaccine-lane membership fix):** 28 closed, 4 outstanding, 2 maintainer decisions.
(BUG-040 fixed 2026-07-24 — partition-label canonicalization; corrects the BUG-029/BUG-035 "partition-aware" claim, which was a silent no-op for numeric-partition sheds.)
(BUG-041 FIXED 2026-07-24 — TWO producer paths, fixed in two commits. (1) `db555c6e`: `mergeUnfinalizedBatchIntoPlannedDate` (combo-align) rebuilds the merged target via the real planner. (2) per-rule sweep path: `batchDueGroup` finalize (`sweeper.go`) REPLACED a batch's drive assignments from ONLY the current sweep pass's `attachedRows`, but `batchSession()` maps sibling vaccines onto one combo session and a rule attaches across shed/date chunks, so a batch is shared across passes and the last pass's REPLACE wiped earlier passes' shed/lane cells → the exact CPT-reseed 109-unbound shapes (combo:FMD+HS batch keeping only the HS lane; a rule:<uuid> Blue-Tongue batch keeping only the Old-Yashoda arm, e.g. goat `ec5e4e75…`/RFID 901007000503935). Fix: the per-rule finalize now rebuilds from the FULL attached set via `RebuildMergedBatchDriveAssignments`/`DriveRebuildInputsForBatch` (real operators, target excluded from its own capacity load, no NULL-operator rows), same as the merge path. **Proven on a full CPT reseed of the real fixture (324 goats, throwaway local :5433 DB): unbound 109→0; dup/goat≠target/wrong-vaccine/animal_count≠members/cap>200/NULL-operator/partition-mismatch all 0.** Two new per-rule multi-pass regression tests added (sibling-lane + cross-shed). SEPARATE known gap (not this bug): the ET+TT-then-PPR-after-14-days schedule variant is not implemented — PPR is still co-scheduled with ET+TT on 07-24/07-25 rather than deferred to 08-07.)
(BUG-029 closed resolved-by-design 2026-07-24 — persist SQL binding is deterministic + partition-aware **for "Part N" sheds; numeric-partition sheds were fixed by BUG-040**; ownership split ratified.)
...
| Landed: partition-label canonicalization (CPT reseed proof) | 1 | BUG-040 |
| WONTFIX / accepted risk | 1 | BUG-018 |
| Resolved by design | 1 | BUG-029 |
| Not counted / prior-resolved | 4 | BUG-021, BUG-022, BUG-025, BUG-026 |
| **Open** | **4** | BUG-030 (matrix landed, extensions open), BUG-031, BUG-032 (40 of 58 remain), BUG-041 (planner assignment-coverage gap — blocks stg-seed clean) |
...
Both surfaced while clearing the obligation-adapter baseline (16 → 3). In each case the
agent REFUSED to make the test pass, because passing it would have deleted a real
guarantee. Read these before anyone "fixes" them.

**MD-1 — `TestVisitShotCapAtomicAcrossConcurrentWorkers`: single-pass overflow convergence
regressed (P2, throughput not correctness).** 4 obligations, cap 2, exactly 2 feasible dates
(a perfect fit). One sweep pass now places only 2; the other 2 stay `scheduled` with
`batch_id IS NULL`, and a second identical sweep places both. **Nothing is dropped and the
cap is never breached** — the test's invariant ("none silently dropped") holds. What no
longer holds is its documented SINGLE-PASS resolution. Asserting "2 per pass" would weaken a
real convergence guarantee, so it was left red. Decision needed: is multi-pass convergence
acceptable, or is single-pass a requirement?

**MD-2 — `TestUnboundedPreflightThenSweepStillPartialCommitsWithoutHWM`: the hazard it
guards has been retired.** It asserts a `*ShotCapPriorityTieError` partial commit MUST
reproduce on the unbounded (no-HWM) path. It no longer does: all 3 obligations commit
safely, 2 on `2026-07-01` and HS spaced to `2026-07-15` by `SweepSession.overflowSafeRange`
(`sweep_session.go:299-323`), which applies cross-vaccine live→live immunological spacing
and REPLACES the obligation's own `window_end` for the overflow range. Cap respected,
nothing stranded. Making it pass means inverting the assertion the test exists to make.
**Decision needed, and it is medical, not engineering: may spacing-driven overflow
legitimately land PAST an obligation's `window_end`?**
