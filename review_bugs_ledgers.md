...
(BUG-041 FIXED 2026-07-24 — root cause was `mergeUnfinalizedBatchIntoPlannedDate` not rebuilding the target batch's drive assignments after combo-align merges obligations in; fix rebuilds the target via the real planner with real operators; two throwaway-Postgres regression tests prove 0 unbound + no NULL operators + idempotent for both cross-shed and sibling-lane shapes. CPT-reseed re-proof still owed against real data before stg-seed certification.)
