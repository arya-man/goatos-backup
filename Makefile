...
# commandboard-query-plan-guard: the executable plan gate for GET /vaccination/command and its
# drilldowns.
#
# It EXPLAIN (ANALYZE)s each command-board statement against a seeded fixture and fails when a plan
# does tenant-scale work to produce a drawer-sized answer: a broad scan of a hot table where the
# cell should have been a predicate, a materialised CTE re-scanned per outer row, a nested loop
...
# commandboard-query-plan-wiring-guard: the DB-FREE half, run by `make guardrails`.
#
# The full plan gate needs PostgreSQL, so it belongs to the query-plans CI job rather than to the
# static sweep every machine runs. Its wiring does not: this checks that every test the gate claims
# to require exists, and that every command-board SQL statement has a plan-table entry or an
# explicit measured exemption. Deleting a plan-table row used to leave the gate green.
commandboard-query-plan-wiring-guard:
	bash tools/dev/commandboard-query-plan-guard.sh --self-test
