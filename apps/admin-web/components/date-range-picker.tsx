...
export function DateRangePicker({
  ...
  const [visibleMarkerDates, setVisibleMarkerDates] = useState<readonly string[]>(markerDates);
  const markerDateSet = useMemo(() => new Set(visibleMarkerDates), [visibleMarkerDates]);
  const markerDatesKey = markerDates.join("|");

  useEffect(() => {
    if (!markerFetchPath) {
      setVisibleMarkerDates((current) => (sameDateKeys(current, markerDates) ? current : markerDates));
      return;
    }
    ...
    if (fromKey > today) {
      setVisibleMarkerDates((current) => (current.length === 0 ? current : []));
      return;
    }
    ...
    fetch(url, { cache: "no-store", signal: controller.signal })
      ...
      .then((body: { dates?: string[] }) => {
        if (!controller.signal.aborted) {
          ...
          setVisibleMarkerDates((current) => (sameDateKeys(current, nextDates) ? current : nextDates));
        }
      })
      .catch(() => {
        if (!controller.signal.aborted) setVisibleMarkerDates((current) => (current.length === 0 ? current : []));
      });
    return () => controller.abort();
  }, [cursor, markerDatesKey, markerFetchPath, today]);
