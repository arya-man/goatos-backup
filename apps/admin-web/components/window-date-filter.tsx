 ...
 * windowed analytics screen uses.
 *
 ...
 * than several that drift, which is the same reasoning behind the shared operational-location
 * helper. This wrapper is the second half of that: the optimistic-selection mechanism below
 * is subtle enough that a second hand-written copy would drift from it, so Herd Analytics and
 * Health Analytics share one implementation.
 *
 * It is a PAGE filter, not top-bar scope. Park stays in the top bar (Scope Chrome Rule) and is
 * preserved untouched here: this component only ever writes `from`/`to`, so changing the window
 * can never reset which park the reader is looking at.
 */
export function WindowDateFilter({
  ...
  function apply(nextFrom: string, nextTo: string): void {
    ...
    // is expressed by absence — a bookmark then keeps meaning "the last N months" instead of
    // freezing on the day it was taken. Every other selection, today included, is written
    // into the URL: deleting the params for one would silently fall back to the whole default
    // window, which reads as the filter not working.
