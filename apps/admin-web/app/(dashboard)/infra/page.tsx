...
export default function InfraPage() {
  ...
  const { data: infraResponse, isLoading, error, refetch } = useQuery<{ data: CountingRecord[]; shedCapacities?: Record<string, number> }>({
    queryKey: ["infra", farm],
    queryFn: async () => {
  ...
  const records: CountingRecord[] = useMemo(() => infraResponse?.data ?? [], [infraResponse]);
  const shedCapacities = infraResponse?.shedCapacities;
  ...
  const shedGroups = useMemo(() => {
    ...
    const partitions = buildShedPartitions(records, shedCapacities);
    return groupByShedName(partitions);
  }, [records, shedCapacities]);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {group.partitions.map((partition) => {
                    ...
                    return (
                      <Link
                        key={partition.shed}
                        href={`/infra/${encodeURIComponent(partition.shed)}?farm=${farm}`}
