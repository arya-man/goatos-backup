  ...
  try {
    if (USE_BIGQUERY) {
      const rows = await queryBigQuery(
        `SELECT date, SUM(total_spend) as total_spend FROM ${table("feed_daily_spend")} WHERE date >= DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 30 DAY) AND date < CURRENT_DATE('Asia/Kolkata') GROUP BY date ORDER BY date`
      );
