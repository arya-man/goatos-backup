...
return NextResponse.json({
    data: {
      ...
      kiddingEfficiency: (() => {
        ...
        // Auto-detect if values are decimals (all <= 1) and convert to percentages
        const allDecimal = mapped.length > 0 && mapped.every((m) => m.pct <= 1);
        if (allDecimal) {
          return mapped.map((m) => ({ ...m, pct: round2(m.pct * 100) }));
        }
        return mapped;
      })(),
