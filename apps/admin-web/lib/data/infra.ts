...
export function buildShedPartitions(records: CountingRecord[], shedCapacities?: Record<string, number>): ShedPartition[] {
  ...
  return Array.from(map.entries()).map(([shed, { breeds, shedTag }]) => {
    ...
    return {
      ...
      breeds: breedArr,
      totalCount,
      capacity: (shedCapacities && shedCapacities[shed] != null) ? shedCapacities[shed] : getShedCapacity(shed),
    };
  });
}
