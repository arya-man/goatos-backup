...
export async function FeedConfigPage({
  ...
  const [
    ...
    listFeedConfigFeedItems({ limit: SECONDARY_PAGE_SIZE }),
  ]);
  ...
  // Filter vocabularies come from the rows the park actually has, so an option can never match zero
  // rows. Sorted for a stable order across pages.
  ...
  const filterFields: FeedFilterField[] = [
    ...
    },
    {
      ...
      options: uniqueSorted(gridRows.map((row) => row.ration_group)).map((value) => ({ value, label: value })),
    },
    {
      kind: "select",
      param: "fc_tag",
      ...
      value: shedTagFilter,
      options: uniqueSorted(gridRows.map((row) => row.shed_tag)).map((value) => ({ value, label: value })),
    },
    {
      kind: "select",
      param: "fc_item",
      ...
      options: uniqueSorted(gridRows.map((row) => row.feed_item)).map((value) => ({ value, label: value })),
    },
  ];
