...
// in :app (PcCareWorklistViewModel / PcCareTaskViewModel / PcCarePlanViewModel), which wire
// AnalyticsPort events for every read refresh, scan, capture, and submit.
...
android {
    namespace = "sg.mesha.goatos.feature.pccare"
    compileSdk = 36
    defaultConfig {
        minSdk = 29
    }
...
dependencies {
    ...
    // Paging's Compose bindings: the worklist and planner monitor render LazyPagingItems so each
    // list is bounded at both layers (Room window + network page), never a materialized full list.
