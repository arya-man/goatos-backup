...
fun FeedTransportScreen(
    ...
    Column(
        Modifier.fillMaxSize().background(MeshaColors.PageBg).padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(12.dp),
    ) {
        Text("Feed Transport", fontSize = 22.sp, fontWeight = FontWeight.Bold, color = MeshaColors.Ink)
        Text("Today · ${state.date} · Starts 15:30", color = MeshaColors.Muted)
        LazyColumn(
            state = listState,
            ...
            items(state.rows, key = { it.taskId }) { row ->
                Column(
                    Modifier.fillMaxWidth().background(MeshaColors.Surf).clickable(
                        enabled = row.status == "due" || row.status == "rework",
                    ) { onEvent(FeedTransportEvent.Open(row)) }.padding(14.dp),
                ) {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        Text(row.shedLabel, fontWeight = FontWeight.Bold, color = MeshaColors.Ink)
                        Text(row.status.replace('_', ' '), color = MeshaColors.Muted)
                    }
                    Text(row.parkLabel, color = MeshaColors.Faint)
                    row.reworkReason?.takeIf { it.isNotBlank() }?.let {
                        Text("Rework: $it", color = MeshaColors.Danger)
                    }
                }
            }
            if (state.isLoadingMore) {
                item(key = "feed-transport-loading") {
                    Text("Loading…", color = MeshaColors.Muted)
                }
            }
        }
    }
