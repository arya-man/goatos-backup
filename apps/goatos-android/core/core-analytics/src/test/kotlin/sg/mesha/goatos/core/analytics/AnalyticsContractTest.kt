...
class AnalyticsContractTest {
    ...
    fun `firebase event params keep the surviving proof-flow telemetry params within the 25-cap`() {
        ...
        // feed_weight_source, feed_video_source, water_video_source, request_token, and prompt are
        // dropped from the Firebase envelope; previous/next/status survive, and the full payload
        // still reaches the backend mirror via BackendAnalyticsAdapter on the same call site. This
        // test proves the adapter output actually contains every SURVIVING param, not just that the
        // constants exist, and that the dropped ones are genuinely dropped.
        val params = firebaseEventParams(
            mapOf(
        ...
        assertNull("dropped to fit the 25-cap; full value still reaches the backend mirror", params[AnalyticsEvents.Params.FEED_WEIGHT_SOURCE])
        assertNull("dropped to fit the 25-cap; full value still reaches the backend mirror", params["request_token"])
        assertNull("dropped to fit the 25-cap; full value still reaches the backend mirror", params["prompt"])
    }
