 ...
 *  immediately instead of burning the full backoff budget, while keeping timeout/rate-limit
...
class RetryClassificationTest {
    ...
    @Test
    fun `definitive auth access and payload 4xx client errors are terminal`() {
    ...
    @Test
    fun `timeout and rate-limit 4xx stay retryable`() {
        listOf(408, 425, 429).forEach { code ->
