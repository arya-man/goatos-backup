...
class RoomScreenCacheStoreTest {
    ...
    fun `clearAll empties every cache table, including bootstrap and roster tables`() = runTest {
        ...
        try {
            ...
            db.scanRosterCacheDao().upsert(ScanRosterCacheEntity(cacheKey = key, dtoJson = "{}", updatedAt = 1L))
            ...
            assertNull("scan roster cache wiped", db.scanRosterCacheDao().observe(key).first())
