    ...
    private suspend fun processItem(item: OutboxEntity, rememberRetryDue: (Long) -> Unit): Boolean {
        ...
        return try {
            ...
            if (store.markSucceeded(item.id, resultJson, clock())) {
                // Carries the result the row was just marked with. `item` is the entity as it
                // was read BEFORE the write, so its resultJson is still null; a reconcile that
                // projects the server's answer would see nothing here while the recovery path
                // (observeRecentTerminals, which re-reads the row) saw it -- the same hook
                // behaving differently on its two callers.
