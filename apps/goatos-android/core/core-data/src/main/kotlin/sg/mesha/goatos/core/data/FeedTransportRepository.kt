...
class FeedTransportRepository(private val api:AppApi,private val db:GoatDatabase,private val json:Json=Json{ignoreUnknownKeys=true},private val clock:()->Long={System.currentTimeMillis()}){
    fun observe(date:String,limit:Int):Flow<FeedTransportTaskPageDto> = combine(db.feedTransportItemDao().observe(date,limit),db.feedTransportRemoteKeyDao().observe(date)){rows,key->FeedTransportTaskPageDto(rows.map{json.decodeFromString<FeedTransportTaskDto>(it.dtoJson)},key?.nextCursor)}
