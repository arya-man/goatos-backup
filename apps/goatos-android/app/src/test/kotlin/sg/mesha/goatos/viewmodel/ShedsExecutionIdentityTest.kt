...
class ShedsExecutionIdentityTest {
    ...
    @Test
    fun `sibling exact sheds under one group produce distinct card keys`() {
        val part1 = executionCardId("shed-castro-1", "task-a", "batch-a", null, "Part 1")
        val part2 = executionCardId("shed-castro-2", "task-a", "batch-a", null, "2")

        assertEquals("shed:shed-castro-1|task:task-a", part1)
        assertEquals("shed:shed-castro-2|task:task-a", part2)
    ...
    fun `execution card key ignores backend row metadata splits`() {
        ...
        assertEquals("shed:shed-84|task:task-bt-sp", bluetongue.executionCardId())
