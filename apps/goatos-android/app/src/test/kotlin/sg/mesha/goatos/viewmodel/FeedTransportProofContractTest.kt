...
/**
 * Proof flow contract test: feed transport video.
 *
 * Tests the complete lifecycle for feed transport proof:
...
class FeedTransportProofContractTest {
    ...
    @Test
    fun `feed transport proof: complete chain from capture to submitted`() = runTest(dispatcher) {
        val taskId = "feed-transport:2026-08-13:shed-1"
        val shedId = "shed-1"
        val proofUri = "/capture/feed-transport-001.mp4"

        // ========== STEP 1: Capture transport video ==========
        val captureResult = proofRepository.capture(
            taskId = taskId,
            fieldKey = "feed_transport_video",
        ...
        val recaptureUri = "/capture/feed-transport-retake.mp4"
        ...
        // ========== STEP 9: Submit ==========
        val proofIdentity = ProofIdentity.feedTransportProofIdentity(
            taskId = taskId,
            shedId = shedId,
        )
        ...
        assertTrue(
            ...
            idempotencyKey.contains("feed-transport") && idempotencyKey.contains(shedId),
        )
    }
}
