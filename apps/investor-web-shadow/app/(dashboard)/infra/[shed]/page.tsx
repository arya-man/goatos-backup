...
export default function ShedDetailPage() {
  ...
  // Fetch both farms and combine
  const { data: cbeRecords = [] } = useQuery<CountingRecord[]>({
    queryKey: ["infra", "CBE"],
    queryFn: () => fetch("/api/infra?farm=CBE").then((r) => r.json()).then((j) => j.data),
    staleTime: 15 * 60 * 1000,
  });

  const { data: cptRecords = [] } = useQuery<CountingRecord[]>({
    queryKey: ["infra", "CPT"],
    queryFn: () => fetch("/api/infra?farm=CPT").then((r) => r.json()).then((j) => j.data),
    staleTime: 15 * 60 * 1000,
  });

  const allRecords = useMemo(
    () => [...cbeRecords, ...cptRecords],
    [cbeRecords, cptRecords]
  );

  // Filter for this shed
  const shedRecords = useMemo(
    () => allRecords.filter((r) => r.shed === shedName),
    [allRecords, shedName]
  );
    ...
    <div className="space-y-6">
      ...
      {breedCounts.length > 0 && (
        <ChartCard title="Breed Distribution" subtitle={`${breedCounts.length} breeds in this shed`}>
          <HorizontalBarChart data={breedCounts} valueLabel="Count" />
        </ChartCard>
      )}
