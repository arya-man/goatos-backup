...
export default function ShiftingsPage() {
  ...
  const { data: allRecords = [], isLoading, error, refetch } = useQuery<ShiftingRecord[]>({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/shiftings?farm=${farmTab}`);
  ...
  return (
    <div className="space-y-6">
      ...
      {error && <ErrorState message="Failed to load shiftings data" onRetry={() => refetch()} />}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#334155] bg-[#1A1D24]">
                  <th className="px-3 py-2.5 text-left font-medium text-[#8899AA] w-12">#</th>
                  <th className="px-3 py-2.5 text-left font-medium text-[#8899AA]">Goat ID</th>
                  <th className="px-3 py-2.5 text-left font-medium text-[#8899AA]">Stage</th>
                  <th className="px-3 py-2.5 text-left font-medium text-[#8899AA]">Days in Current Stage</th>
                </tr>
              </thead>
              <tbody>
                {records.length === 0 && (
