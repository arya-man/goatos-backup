  ...
  try {
    ...
    if (USE_BIGQUERY) {
      ...
      const [
        ...
        queryBigQuery(`SELECT * FROM ${table("overall_farmwise_mortality_dev")}`),
      ...
      const farmwise = filteredFarmRows.map((r: any) => ({
        ...
        kidMortality: round2(r.kid_deaths_share_of_total_pct),
        kidAbortion: round2(r.abortions_share_of_total_pct),
        adultMortality: round2(r.adult_deaths_share_of_total_pct),
        totalMortality: round2(r.total_deaths_share_of_total_pct),
      }));

      // "Within Farm" = % of animals within that farm that died
      const farmwiseWithin = filteredFarmRows.map((r: any) => ({
        farm: r.farm || r.shed_tag || '',
        kidMortality: round2(r.kid_mortality_pct_overall),
        kidAbortion: round2(r.abortion_pct_overall),
        adultMortality: round2(r.adult_mortality_pct_overall),
        totalMortality: round2(r.farm_mortality_pct_overall),
      }));
