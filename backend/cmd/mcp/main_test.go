...
func TestHealthTodayCombinesAdultAndKid(t *testing.T) {
	seen := map[string]bool{}
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/app/health/work-items" {
			t.Fatalf("path=%s", r.URL.Path)
		}
		ageBand := r.URL.Query().Get("age_band")
		seen[ageBand] = true
		if r.URL.Query().Get("date") != "2026-08-15" {
	...
	req := httptest.NewRequest(http.MethodPost, "/mcp", strings.NewReader(`{"jsonrpc":"2.0","id":"api","method":"tools/call","params":{"name":"get_health_today","arguments":{"date":"2026-08-15","limit":20}}}`))
	req.Header.Set("Authorization", "Bearer user-token")
	rec := httptest.NewRecorder()
	...
	if !seen["adult"] || !seen["kid"] {
		t.Fatalf("seen age bands=%v", seen)
	}
	body := rec.Body.String()
	if !strings.Contains(body, `"adult"`) || !strings.Contains(body, `"kid"`) || !strings.Contains(body, `"structuredContent"`) {
