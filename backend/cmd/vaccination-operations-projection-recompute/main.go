...
func run(args []string) error {
	fs := flag.NewFlagSet("vaccination-operations-projection-recompute", flag.ContinueOnError)
	...
	if env == "stg" {
		err = localtarget.ValidateStagingCloudSQLDatabaseTarget("vaccination-operations-projection-recompute", env, cfg.DatabaseURL)
	} else {
		err = localtarget.ValidateLocalDatabaseTarget("vaccination-operations-projection-recompute", env, cfg.DatabaseURL, "local", "dev")
	}
	if err != nil {
		return err
	}
	...
	result, err := vaccexecpg.NewRepository(pool, cfg.QueryTimeout).RecomputeOperationsProjection(ctx, domain.OperationsProjectionRecomputeRequest{TenantID: *tenantID, AsOf: asOf})
	if err != nil {
		return err
	}
	fmt.Printf("recomputed vaccination operations projection tenant=%s rows=%d version=%d as_of=%s\n", result.TenantID, result.Rows, result.ProjectionVersion, result.AsOf.Format(time.RFC3339))
	return nil
}
