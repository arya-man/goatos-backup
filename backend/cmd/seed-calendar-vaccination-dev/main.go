...
INSERT INTO inventory_stock (
  ...
  '` + stockID + `', $1::uuid, '` + itemID + `', '` + cbeParkID + `', 'CAL-ET-001',
...
VALUES
  ('` + protocolID + `', $1::uuid, 'vaccination.calendar.enterotoxaemia', 'Calendar Enterotoxaemia', 'vaccination', 'active'),
  ('` + draftProtocolID + `', $1::uuid, 'vaccination.calendar.draft_label_only', 'Draft Label Only Vaccine', 'vaccination', 'active')
...
INSERT INTO protocol_versions (
  ...
  ('` + draftVersionID + `', $1::uuid, '` + draftProtocolID + `', 'park', '` + cbeParkID + `', 1,
   'Draft label-only excluded seed', 'draft', DATE '2026-01-01', DATE '2028-01-01',
   '{"source":{"review_status":"draft","seed":"calendar-negative"}}'::jsonb,
   '{}'::jsonb,
   NULL, NULL)
...
SELECT
  ...
  'Enterotoxaemia Vaccine', 'ET-K1-PRIMARY', true, source_label, target_type,
...
INSERT INTO calendar_event_projections (
  ...
  ($1::uuid, 'calendar:86000000-0000-4000-8000-000000009001', 'vaccination', 'vaccination_dose_due', 'phc',
