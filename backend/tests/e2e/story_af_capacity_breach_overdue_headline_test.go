...
func TestKernelStoryAF_CapacityBreachKeepsOverdueHeadline(t *testing.T) {
	...
	// 5k-50k envelope: ShedSummary reads canonical tables directly (no projection read model). No
	// fixture mutation happened since shedRow's read above, so the same canonical state as of
	// lateAsOf already answers these three direct filter reads correctly.
