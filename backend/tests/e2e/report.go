// Package e2e hosts the vaccination "kernel story" end-to-end test harness: full flows that seed a
// clean-slate scenario against an ephemeral Postgres instance per story, drive the real
// generation -> obligation -> batching/drive -> execution/proof/verification -> read-model chain
// through the same application services and repositories the rest of the backend uses, and render
// the outcome into a single, dependency-free HTML report for engineering/CEO visibility.
//
// See harness_test.go for how each story is run, and run.sh / the package doc comment there for the
// exact command.
package e2e
...
// StepResult is one narrated step of a story (a setup action, a kernel transition, or a read-model
...
<head>
<meta charset="utf-8">
<title>Vaccination Kernel Story Report</title>
<style>
...
<body>
  ...
  <footer>Generated by backend/tests/e2e &mdash; run via <code>backend/tests/e2e/run.sh</code> or <code>go test ./backend/tests/e2e/... -run TestKernelStor -v</code>.</footer>
</body>
</html>
`))
