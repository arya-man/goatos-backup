#!/usr/bin/env bash
# Runs the HRMS "kernel story" E2E suite (backend/tests/e2e-hrms) and prints the HTML report path.
#
# This script cd's to the repo root itself, so it can be invoked from anywhere:
#   backend/tests/e2e-hrms/run.sh
#
...
# backend/tests/e2e-hrms/report/index.html (open directly in a browser, or host as-is on GitHub Pages).
set -euo pipefail
...
REPORT="$REPO_ROOT/backend/tests/e2e-hrms/report/index.html"
if [ -f "$REPORT" ]; then
  echo ""
  echo "HRMS Kernel story report: file://$REPORT"
else
