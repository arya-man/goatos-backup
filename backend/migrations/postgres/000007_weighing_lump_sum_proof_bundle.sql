...
ALTER TABLE public.weighing_shed_observations
  ADD COLUMN IF NOT EXISTS average_weight_kg numeric(8,3);

UPDATE public.weighing_shed_observations
SET average_weight_kg = weight_kg
WHERE average_weight_kg IS NULL;

ALTER TABLE public.weighing_shed_observations
  ALTER COLUMN average_weight_kg SET NOT NULL;

ALTER TABLE public.weighing_shed_observations
  DROP CONSTRAINT IF EXISTS weighing_shed_observations_average_weight_check;

ALTER TABLE public.weighing_shed_observations
  ADD CONSTRAINT weighing_shed_observations_average_weight_check
  CHECK (average_weight_kg > 0);
