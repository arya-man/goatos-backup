...
--
-- Name: vaccination_schedule_projection_state; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.vaccination_schedule_projection_state (
    tenant_id uuid NOT NULL,
    scope_type text NOT NULL,
    ...
    generated_by text,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT vaccination_schedule_projection_state_counts_check CHECK ((row_count >= 0)),
    CONSTRAINT vaccination_schedule_projection_state_freshness_check CHECK ((freshness_status = ANY (ARRAY['green'::text, 'yellow'::text, 'red'::text, 'unknown'::text]))),
    CONSTRAINT vaccination_schedule_projection_state_scope_check CHECK ((scope_type = ANY (ARRAY['tenant'::text, 'park'::text]))),
    CONSTRAINT vaccination_schedule_projection_state_serving_check CHECK ((serving_state = ANY (ARRAY['never_synced'::text, 'fresh'::text, 'stale'::text, 'rebuilding'::text, 'failed'::text, 'source_unavailable'::text]))),
    CONSTRAINT vaccination_schedule_projection_state_version_check CHECK ((projection_version >= 1))
);

ALTER TABLE ONLY public.vaccination_schedule_projection_state
    ADD CONSTRAINT vaccination_schedule_projection_state_pk PRIMARY KEY (tenant_id, scope_type, scope_id, year_month);


--
-- Name: vaccination_schedule_projection_rows; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.vaccination_schedule_projection_rows (
    tenant_id uuid NOT NULL,
    scope_type text NOT NULL,
    scope_id uuid DEFAULT '00000000-0000-0000-0000-000000000000'::uuid NOT NULL,
    year_month date NOT NULL,
    ...
    vaccine_names text[] DEFAULT ARRAY[]::text[] NOT NULL,
    ...
    proof_pending_count integer DEFAULT 0 NOT NULL,
    rejected_count integer DEFAULT 0 NOT NULL,
    total_count integer DEFAULT 0 NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT vaccination_schedule_projection_rows_counts_check CHECK (((animals >= 0) AND (overdue_count >= 0) AND (due_count >= 0) AND (in_progress_count >= 0) AND (scheduled_count >= 0) AND (missed_count >= 0) AND (deferred_count >= 0) AND (accepted_count >= 0) AND (proof_pending_count >= 0) AND (rejected_count >= 0) AND (total_count >= 0))),
    CONSTRAINT vaccination_schedule_projection_rows_scope_check CHECK ((scope_type = ANY (ARRAY['tenant'::text, 'park'::text]))),
    CONSTRAINT vaccination_schedule_projection_rows_version_check CHECK ((projection_version >= 1))
);

ALTER TABLE ONLY public.vaccination_schedule_projection_rows
    ADD CONSTRAINT vaccination_schedule_projection_rows_pk PRIMARY KEY (tenant_id, scope_type, scope_id, year_month, projection_version, park_id, shed_id, stage, protocol_id);

CREATE INDEX vaccination_schedule_projection_rows_lookup_idx ON public.vaccination_schedule_projection_rows USING btree (tenant_id, scope_type, scope_id, year_month, projection_version, park_name, shed_name, stage);


--
-- Name: vaccination_schedule_projection_dirty_scopes; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.vaccination_schedule_projection_dirty_scopes (
    dirty_scope_id uuid DEFAULT gen_random_uuid() NOT NULL,
    tenant_id uuid NOT NULL,
    scope_type text NOT NULL,
    scope_id uuid DEFAULT '00000000-0000-0000-0000-000000000000'::uuid NOT NULL,
    year_month date NOT NULL,
    ...
    dirty_count integer DEFAULT 1 NOT NULL,
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT vaccination_schedule_dirty_counts_check CHECK ((dirty_count >= 1)),
    CONSTRAINT vaccination_schedule_dirty_scope_check CHECK ((scope_type = ANY (ARRAY['tenant'::text, 'park'::text, 'shed'::text]))),
    CONSTRAINT vaccination_schedule_dirty_status_check CHECK ((status = ANY (ARRAY['dirty'::text, 'rebuilding'::text, 'rebuilt'::text, 'failed'::text]))),
    CONSTRAINT vaccination_schedule_dirty_reason_check CHECK ((btrim(reason) <> ''::text))
);

ALTER TABLE ONLY public.vaccination_schedule_projection_dirty_scopes
    ADD CONSTRAINT vaccination_schedule_projection_dirty_scopes_pk PRIMARY KEY (dirty_scope_id);

CREATE UNIQUE INDEX vaccination_schedule_projection_dirty_scope_key ON public.vaccination_schedule_projection_dirty_scopes (tenant_id, scope_type, scope_id, year_month);
CREATE INDEX vaccination_schedule_projection_dirty_pending_idx ON public.vaccination_schedule_projection_dirty_scopes USING btree (tenant_id, status, year_month, scope_type, scope_id) WHERE (status = 'dirty'::text);


--
-- Name: vaccination_schedule_month_start(timestamp with time zone); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.vaccination_schedule_month_start(p_anchor timestamp with time zone) RETURNS date
    LANGUAGE sql IMMUTABLE
    AS $$
  SELECT (date_trunc('month', p_anchor AT TIME ZONE 'Asia/Kolkata'))::date;
$$;


--
-- Name: vaccination_schedule_mark_dirty(uuid,text,uuid,timestamp with time zone,text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.vaccination_schedule_mark_dirty(p_tenant_id uuid, p_scope_type text, p_scope_id uuid, p_anchor timestamp with time zone, p_reason text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  month_start date;
  dirty_scope_type text := CASE WHEN p_scope_type IN ('park', 'shed') AND p_scope_id IS NOT NULL THEN p_scope_type ELSE 'tenant' END;
BEGIN
  IF p_tenant_id IS NULL OR p_anchor IS NULL THEN
    RETURN;
  END IF;
  month_start := public.vaccination_schedule_month_start(p_anchor);
  INSERT INTO vaccination_schedule_projection_dirty_scopes (
    tenant_id, scope_type, scope_id, year_month, reason
  ) VALUES (
    p_tenant_id,
    dirty_scope_type,
    CASE WHEN dirty_scope_type = 'tenant' THEN '00000000-0000-0000-0000-000000000000'::uuid ELSE p_scope_id END,
    month_start,
    COALESCE(NULLIF(p_reason, ''), 'unknown')
  )
  ON CONFLICT (tenant_id, scope_type, scope_id, year_month) DO UPDATE SET
    status = 'dirty',
    dirty_count = vaccination_schedule_projection_dirty_scopes.dirty_count + 1,
...
CREATE FUNCTION public.vaccination_schedule_dirty_obligation_trg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  rec record;
...
BEGIN
  FOR rec IN
    SELECT (to_jsonb(NEW.*)) AS data WHERE TG_OP <> 'DELETE'
    UNION ALL
    SELECT (to_jsonb(OLD.*)) AS data WHERE TG_OP <> 'INSERT'
  LOOP
    IF rec.data ->> 'target_type' <> 'goat' THEN
      CONTINUE;
    END IF;
    SELECT g.shed_id INTO shed
    FROM goats g
    WHERE g.tenant_id = (rec.data ->> 'tenant_id')::uuid
      AND g.goat_id = (rec.data ->> 'target_id')::uuid;
    PERFORM public.vaccination_schedule_mark_dirty(
      (rec.data ->> 'tenant_id')::uuid,
      CASE WHEN shed IS NULL THEN 'tenant' ELSE 'shed' END,
      shed,
      (rec.data ->> 'due_at')::timestamptz,
      'obligation_instances.' || lower(TG_OP)
    );
  END LOOP;
  RETURN COALESCE(NEW, OLD);
END;
...
CREATE TRIGGER vaccination_schedule_dirty_obligation_after_write_trg
AFTER INSERT OR UPDATE OF due_at, window_start, window_end, status, completed_at, batch_id, sop_task_id, target_id, rule_id, protocol_version_id OR DELETE ON public.obligation_instances
FOR EACH ROW EXECUTE FUNCTION public.vaccination_schedule_dirty_obligation_trg();


CREATE FUNCTION public.vaccination_schedule_dirty_completion_trg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  rec record;
...
BEGIN
  FOR rec IN
    ...
    SELECT (to_jsonb(OLD.*)) AS data WHERE TG_OP <> 'INSERT'
  LOOP
    SELECT oi.due_at, g.shed_id INTO due_at, shed
    FROM obligation_instances oi
    LEFT JOIN goats g
      ON g.tenant_id = oi.tenant_id
     AND g.goat_id = oi.target_id
    WHERE oi.tenant_id = (rec.data ->> 'tenant_id')::uuid
      AND oi.obligation_id = (rec.data ->> 'obligation_id')::uuid;
    PERFORM public.vaccination_schedule_mark_dirty(
      (rec.data ->> 'tenant_id')::uuid,
      CASE WHEN shed IS NULL THEN 'tenant' ELSE 'shed' END,
      shed,
      COALESCE((rec.data ->> 'administered_at')::timestamptz, due_at),
      'vaccination_completions.' || lower(TG_OP)
    );
  END LOOP;
  RETURN COALESCE(NEW, OLD);
END;
...
CREATE TRIGGER vaccination_schedule_dirty_completion_after_write_trg
AFTER INSERT OR UPDATE OF administered_at, status, verified_at, obligation_id, batch_id OR DELETE ON public.vaccination_completions
FOR EACH ROW EXECUTE FUNCTION public.vaccination_schedule_dirty_completion_trg();


CREATE FUNCTION public.vaccination_schedule_dirty_batch_trg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  PERFORM public.vaccination_schedule_mark_dirty(
    COALESCE(NEW.tenant_id, OLD.tenant_id),
    CASE WHEN COALESCE(NEW.scope_type, OLD.scope_type) IN ('park', 'shed') THEN COALESCE(NEW.scope_type, OLD.scope_type) ELSE 'tenant' END,
    CASE WHEN COALESCE(NEW.scope_type, OLD.scope_type) IN ('park', 'shed') THEN COALESCE(NEW.scope_id, OLD.scope_id) ELSE NULL END,
    COALESCE(NEW.window_start, OLD.window_start, (COALESCE(NEW.planned_date, OLD.planned_date))::timestamp with time zone, now()),
    'obligation_batches.' || lower(TG_OP)
  );
  RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE TRIGGER vaccination_schedule_dirty_batch_after_write_trg
AFTER INSERT OR UPDATE OF status, planned_date, window_start, window_end, scope_type, scope_id OR DELETE ON public.obligation_batches
FOR EACH ROW EXECUTE FUNCTION public.vaccination_schedule_dirty_batch_trg();


CREATE FUNCTION public.vaccination_schedule_dirty_tenant_months(p_tenant_id uuid, p_reason text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  i integer;
...
BEGIN
  FOR i IN 0..13 LOOP
    anchor := (date_trunc('month', now() AT TIME ZONE 'Asia/Kolkata') + (i || ' months')::interval)::timestamp AT TIME ZONE 'Asia/Kolkata';
    PERFORM public.vaccination_schedule_mark_dirty(p_tenant_id, 'tenant', NULL, anchor, p_reason);
  END LOOP;
END;
$$;

CREATE FUNCTION public.vaccination_schedule_dirty_protocol_trg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  PERFORM public.vaccination_schedule_dirty_tenant_months(COALESCE(NEW.tenant_id, OLD.tenant_id), 'protocol_versions.' || lower(TG_OP));
  RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE TRIGGER vaccination_schedule_dirty_protocol_after_write_trg
AFTER INSERT OR UPDATE OF status, published_at, rule_dsl OR DELETE ON public.protocol_versions
FOR EACH ROW EXECUTE FUNCTION public.vaccination_schedule_dirty_protocol_trg();


CREATE FUNCTION public.vaccination_schedule_dirty_workforce_trg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF COALESCE(NEW.scope_type, OLD.scope_type) IN ('shed', 'center') THEN
    PERFORM public.vaccination_schedule_dirty_tenant_months(COALESCE(NEW.tenant_id, OLD.tenant_id), 'workforce_positions.' || lower(TG_OP));
  END IF;
  RETURN COALESCE(NEW, OLD);
END;
$$;

CREATE TRIGGER vaccination_schedule_dirty_workforce_after_write_trg
AFTER INSERT OR UPDATE OF workforce_member_id, scope_type, scope_id, status, valid_from, valid_to, position_code, is_backup_slot OR DELETE ON public.workforce_positions
FOR EACH ROW EXECUTE FUNCTION public.vaccination_schedule_dirty_workforce_trg();
