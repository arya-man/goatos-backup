...
BEGIN
  ...
  IF NEW.aggregate_type = 'weighing' THEN
    IF NOT EXISTS (
      ...
      WHERE tenant_id = NEW.tenant_id
        AND campaign_id = NEW.aggregate_id
      UNION ALL
      SELECT 1
      FROM weighing_observations
      WHERE tenant_id = NEW.tenant_id
        AND observation_id = NEW.aggregate_id
      UNION ALL
      SELECT 1
      FROM weighing_shed_observations
      WHERE tenant_id = NEW.tenant_id
        AND shed_observation_id = NEW.aggregate_id
    ) THEN
...
-- +goose Down
DROP INDEX IF EXISTS public.weighing_campaigns_one_active_week_per_park_idx;
