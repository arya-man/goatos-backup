...
VALUES
  ...
  ($6::uuid, $1::uuid, $2::uuid, $3::uuid, '', '2026-08-12', '2026-08-12 15:30 Asia/Kolkata', 'completed', NULL)`,
		tenant, park, shed, due, pending, rework)
...
ORDER BY business_date`, tenant)
	...
	if got[due] != "due" || got[pending] != "verification_due" || got[rework] != "rework" {
		t.Fatalf("recovered statuses = %#v, want due/verification_due/rework", got)
	}
