...
ALTER TABLE user_scope_grants
  ...
  CHECK (role IN ('admin', 'park_head', 'phc_director', 'operator', 'verifier', 'ceo_internal'));
...
ALTER TABLE workforce_members
  ...
  CHECK (primary_role_hint IN ('operator', 'park_head', 'phc_director', 'verifier', 'supervisor', 'admin', 'other'));
...
ALTER TABLE obligation_escalations
  ...
  CHECK (escalated_to_role IS NULL OR escalated_to_role IN ('admin', 'park_head', 'phc_director', 'operator', 'verifier', 'ceo_internal'));
-- +goose StatementEnd
...
WHERE primary_role_hint = 'phc_director';

UPDATE obligation_escalations
SET escalated_to_role = 'admin'
WHERE escalated_to_role = 'phc_director';
