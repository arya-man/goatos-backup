...
CREATE INDEX CONCURRENTLY IF NOT EXISTS obligation_instances_vaccination_drive_day_idx
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS vaccination_drive_assignments_day_shed_idx
  ON public.vaccination_drive_assignments (tenant_id, planned_date, shed_id);

-- Field evidence for the day.
CREATE INDEX CONCURRENTLY IF NOT EXISTS proof_artifacts_vaccination_day_idx
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS proof_artifacts_vaccination_created_day_idx
  ON public.proof_artifacts (tenant_id, created_at)
  WHERE subject_type = 'goat' AND proof_type = 'video' AND uploaded_at IS NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS sop_task_scan_captures_day_idx
  ON public.sop_task_scan_captures (tenant_id, captured_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS sop_task_scan_attempts_day_idx
  ON public.sop_task_scan_attempts (tenant_id, captured_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS vaccination_completions_day_idx
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS obligation_status_events_day_idx
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS verification_items_pending_scope_idx
  ON public.verification_items (tenant_id, park_id, shed_id)
  WHERE status = 'pending';

CREATE INDEX CONCURRENTLY IF NOT EXISTS verification_items_verified_day_idx
  ON public.verification_items (tenant_id, verified_at)
  WHERE verified_at IS NOT NULL;

-- +goose Down
-- +goose NO TRANSACTION
DROP INDEX CONCURRENTLY IF EXISTS public.verification_items_verified_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.verification_items_pending_scope_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.obligation_status_events_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.vaccination_completions_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.sop_task_scan_attempts_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.sop_task_scan_captures_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.proof_artifacts_vaccination_created_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.proof_artifacts_vaccination_day_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.vaccination_drive_assignments_day_shed_idx;
DROP INDEX CONCURRENTLY IF EXISTS public.obligation_instances_vaccination_drive_day_idx;
