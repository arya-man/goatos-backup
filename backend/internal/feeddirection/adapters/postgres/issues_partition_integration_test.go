...
// issuedAtFixture is the scheduled 07:00 IST dispatch instant for the fixture's feed day. A feed
// day is a BUSINESS DAY, never a clock instant, so this is pinned rather than time.Now(): a
// wall-clock anchor makes the test pass or fail depending on the hour it runs at.
func issuedAtFixture() time.Time {
	return time.Date(2026, 7, 29, 7, 0, 0, 0, biztime.DefaultLocation())
}
...
func TestPersistIssueFreezesEachPartitionOfOneShedAsItsOwnCell(t *testing.T) {
	...
	if _, err := repo.PersistIssue(ctx, issueCmd(cells, "fp-partitions", issuedAtFixture())); err != nil {
