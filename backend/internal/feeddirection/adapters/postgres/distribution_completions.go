...
// ListDistributionSessionStatuses returns EVERY (shed, session, workflow) with a
// feed_distribution_completions row for one park-day plus its RAW status -- the direction serve
// path's status overlay + filter source (includes pending_verification and rework, not just
// completed).
func (r *Repository) ListDistributionSessionStatuses(ctx context.Context, tenantID, parkID string, targetDate time.Time) ([]ports.SessionCompletionStatus, error) {
	ctx, cancel := context.WithTimeout(ctx, r.timeout)
	defer cancel()
...
WHERE tenant_id = $1::uuid AND park_id = $2::uuid AND target_date = $3::date`,
		tenantID, parkID, targetDate.Format("2006-01-02"))
	if err != nil {
		return nil, fmt.Errorf("feeddirection: list distribution session statuses: %w", err)
	}
	defer rows.Close()
