...
// alertCursor is the keyset over the feed's own order: newest first, broken by the request id so
// two alerts stamped in the same instant still page stably.
...
// listAlertsSQL reads the feed lifecycle notifications already routed to one person. It is the
// feed twin of the weighing/vaccination feeds and reads the SAME shared plumbing table, differing
// only in the module discriminator.
//
// MODULE DISCRIMINATOR: context->>'message_key' LIKE 'feed.%'. Every feed producer stamps it --
// the shared verification consumer's FEED profile (messageKeyPrefix "feed",
// backend/internal/notificationbridge/verification_notify_consumer.go) writes
// "feed.proof.pending.verifier", "feed.proof.pending.leadership", "feed.proof.approved",
// "feed.proof.rework", and "feed.record.closed" for BOTH gated feed completions (packing and
// distribution) plus feed transport. Weighing/vaccination/counts rows carry their own prefixes and
// are simply not selected.
//
// IDENTITY: context->>'member_id' is the canonical workforce_member_id the consumer resolved
// BEFORE writing, so filtering on it IS the audience check. An operator can never see another
// operator's alert here because they were never a recipient of it.
//
// DEDUPE: QueueRoleNotifications writes one row per recipient DEVICE, so a two-phone operator has
// two rows for one transition. DISTINCT ON the producer's event_key collapses them back to one
// alert.
//
// BOUNDED: the rolling AlertRetentionDays window plus the per-member equality predicate keep this
// off a full-history scan; both are served by notification_requests_feed_alerts_idx (migration
// 000133).
//
// scale-guard:ignore: workforce-scale read -- one person's own feed notifications inside a fixed
// 30-day window, keyset-paginated. Never herd-scale: notification_requests rows are produced per
// work-state transition per recipient device, not per animal.
const listAlertsSQL = `
WITH me AS (
  SELECT COALESCE(
...
mine AS (
  ...
  WHERE nr.tenant_id = $1::uuid
    ...
    AND nr.context->>'message_key' LIKE 'feed.%'
...
// ListAlerts returns one keyset page of the caller's own feed alerts.
func (r *Repository) ListAlerts(
	...
	if err != nil {
		return domain.AlertPage{}, fmt.Errorf("feeddirection: list alerts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&alert.ShedLabel, &alert.ParkID,
		); err != nil {
			return domain.AlertPage{}, fmt.Errorf("feeddirection: scan alert: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return domain.AlertPage{}, fmt.Errorf("feeddirection: list alerts rows: %w", err)
	}
...
// alertDirectionForRole reads the direction off the RECIPIENT ROLE the producing consumer
// stamped, not off the event type. The same transition can travel both ways at once (a rework
// goes down to the operator and up to the director), and each recipient's own row must say which
