...
//
// The rule: an operator's completion no longer completes the session. It records TWO mandatory proofs
// (a feed-distribution video + a water proof) and flips the session to 'pending_verification'; the
...
// (a) A completion missing the water proof (or the distribution video) is rejected at the APP layer
...
// (b) A completion with both proofs writes a 'pending_verification' row -- NOTHING is completed yet.
