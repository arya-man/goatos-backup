...
//
// The rule: an operator's completion no longer completes the session. It records THREE mandatory
// proofs -- a feed-weight PHOTO, a feed-distribution VIDEO and a water-distribution VIDEO (maintainer
// decision 2026-08-11, migration 000151) -- and flips the session to 'pending_verification'; the
...
// (a) A completion missing ANY of the three proofs is rejected at the APP layer before any state
// changes -- there is nothing for a verifier to approve.
func TestCompleteDistributionRequiresAllThreeProofsAtAppLayer(t *testing.T) {
	...
	missingWater := base
	missingWater.FeedWeightProofRef = "proof-weight-0001"
	...
	if _, err := svc.CompleteDistribution(ctx, missingWater); !errors.Is(err, ports.ErrWaterProofRequired) {
		t.Fatalf("missing water proof err = %v, want ErrWaterProofRequired", err)
	}

	missingVideo := base
	missingVideo.FeedWeightProofRef = "proof-weight-0001"
	...
	// The weight photo added on 2026-08-11. This is the case that would silently pass if the new step
	// were wired into the screen but never enforced on the write path.
	missingWeight := base
	missingWeight.FeedWeightProofRef = ""
	missingWeight.DistributionProofRef = "proof-distribution-0001"
	missingWeight.WaterProofRef = "proof-water-0001"
	if _, err := svc.CompleteDistribution(ctx, missingWeight); !errors.Is(err, ports.ErrFeedWeightProofRequired) {
...
WHERE tenant_id = $1::uuid AND completion_id = $2::uuid`, fdTenant, pending.CompletionID).Scan(&status, &rowVersion); err != nil {
	...
	resubmit.FeedWeightProofRef = "proof-weight-0002"
