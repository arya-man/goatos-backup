...
func (r *LocationReader) GoatLocation(ctx context.Context, tenantID, goatID string) (oploc.OperationalLocation, bool, error) {
	...
	var parkID, parkName, shedID, shedName, partitionLabel *string
	err := r.pool.QueryRow(ctx, `
SELECT g.park_id::text, park.name, g.shed_id::text, shed.name, gsp.partition_label
FROM goats g
...
WHERE g.tenant_id = $1::uuid AND g.goat_id = $2::uuid`,
		tenantID, goatID).Scan(&parkID, &parkName, &shedID, &shedName, &partitionLabel)
