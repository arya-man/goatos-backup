...
func CalendarPresentationForQuery(ownerKey string) CalendarPresentation {
	...
	return CalendarPresentation{
		PageTitle:              "Calendar",
		PageSubtitle:           "Vaccination drives by time — one row per shed drive or catch-up drive. Open a drive to page through eligible goats (10 per page).",
		...
		NewEvent:               CalendarActionPresentation{Label: "New event", Enabled: false, DisabledReason: "Calendar events are generated from source-backed obligations. Create a campaign/catch-up via Config or the PHC catch-up path - not a free-form Calendar entry."},
...
func calendarWeekPresentation(active ownerPresentationConfig) CalendarViewPresentation {
	return CalendarViewPresentation{
		...
		EmptyMessage:         "No vaccination due work",
...
var ownerPresentationConfigs = []ownerPresentationConfig{
	{
		...
		workstreams:  []CalendarPresentationTab{activeWorkstream("vaccination", "Vaccination", "Calendar is currently showing the PHC vaccination module.")},
		weekTitle:    "This week",
	...
	},
	{
		...
		monthScope:   "PHC",
		scopeMessage: "Showing PHC work only.",
	},
	{
		key:         OwnerInventory,
