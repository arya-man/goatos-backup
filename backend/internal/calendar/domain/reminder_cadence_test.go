...
func TestLatestDueReminderFirePicksLatestOnCatchUp(t *testing.T) {
	due := istDate(2026, 7, 20, 10, 0)
	// Now is D-1 at 18:30: D-1 08:00, 13:00, and 18:00 are all past due, but nothing has fired
	// yet (fired always false) -- the sweeper should pick the LATEST (18:00), not burst all three.
	now := istDate(2026, 7, 19, 18, 30)
	fire, ok := LatestDueReminderFire(due, now, DefaultReminderLadder(), func(string) bool { return false })
	if !ok {
		t.Fatalf("expected a due fire")
	}
	if fire.Type != ReminderTypeReminder || fire.OffsetDays != -1 || fire.Slot != "18:00" {
		t.Fatalf("fire = %#v, want reminder D-1 18:00 (latest, not the earlier slots)", fire)
	}
}
...
func TestLatestDueReminderFireSkipsAlreadyFired(t *testing.T) {
	...
	firedKeys := map[string]bool{
		"2026-07-19:reminder:18:00": true,
	}
