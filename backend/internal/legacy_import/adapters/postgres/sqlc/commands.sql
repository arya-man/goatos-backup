...
-- name: CountConfirmedNonGoatRowsForDisposition :one
SELECT count(*)::int
FROM legacy_import_rows
WHERE tenant_id = @tenant_id
  AND import_run_id = @import_run_id
...
-- name: ListConfirmedNonGoatRowsForDisposition :many
SELECT
  legacy_row_id::text AS legacy_row_id,
  row_number,
  source_row_key,
...
-- name: RejectLegacyImportRowAsConfirmedNonGoat :execrows
UPDATE legacy_import_rows
SET
...
-- name: InsertAuditLogForImportRowDisposition :exec
INSERT INTO audit_log (
  tenant_id,
  actor_id,
  actor_type,
