...
// TestRegisterVerificationAppliersRegistersAllSeven is the drift guard for the incident this package
...
func TestRegisterVerificationAppliersRegistersAllSeven(t *testing.T) {
	...
	// + weighing = 7 appliers, each subscribing to BOTH verdict types. Weighing and milk preparation each
	...
	const wantAppliers = 7
	for _, eventType := range []string{
		"verification.verdict.approved",
		"verification.verdict.rework",
	} {
		if got := bus.subs[eventType]; got != wantAppliers {
			t.Fatalf("%s subscribers = %d, want %d (shifting + milk-preparation + feed-distribution + feed-packing + feed-transport + feed-wastage + weighing)", eventType, got, wantAppliers)
		}
	}
}
