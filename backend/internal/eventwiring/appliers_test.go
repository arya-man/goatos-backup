...
// TestRegisterVerificationAppliersRegistersAllSix is the drift guard for the incident this package
...
func TestRegisterVerificationAppliersRegistersAllSix(t *testing.T) {
	bus := &spyBus{subs: map[string]int{}}

	RegisterVerificationAppliers(bus, nil, nil, nil, nil, nil, nil)

	// shifting + milk-preparation + feed-distribution + feed-packing + feed-transport + weighing
	// = 6 appliers, each subscribing to BOTH verdict types. Weighing and milk preparation each
	// joined this list because they enqueued a verification item with no consumer at all, so
	// every verdict for them was a silent drop.
	const wantAppliers = 6
	for _, eventType := range []string{
		"verification.verdict.approved",
		"verification.verdict.rework",
	} {
		if got := bus.subs[eventType]; got != wantAppliers {
			t.Fatalf("%s subscribers = %d, want %d (shifting + milk-preparation + feed-distribution + feed-packing + feed-transport + weighing)", eventType, got, wantAppliers)
		}
	}
}
