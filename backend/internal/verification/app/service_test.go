...
// REGRESSION (found on a physical phone, 2026-08-07): the shed filter arriving from
// the client is an oploc.Key() -- "<shed uuid>#<partition>" -- because a partitioned
// shed yields one filter option per partition and a bare uuid cannot tell them apart.
// This validation ran BEFORE the repository that decodes the key, and rejected the
// whole composite as "not a UUID", so every partitioned shed option returned 400.
// Repository-level tests never caught it: they call the repo directly and never cross
// this boundary.
func TestListQueueAcceptsCompositeShedFilterKey(t *testing.T) {
	svc, _ := newTestService()
	const shed = "91000000-0000-4000-8000-000000000201"

	for _, shedFilter := range []string{
		shed,             // bare uuid -- older clients still send this
		shed + "#whole",  // unpartitioned shed's option id
		shed + "#3",      // numeric partition
		shed + "#Part 3", // worded partition
	} {
		if _, err := svc.ListQueue(context.Background(), ports.ListQueueParams{
			...
			t.Fatalf("ListQueue(shed_id=%q) rejected a filter option this service itself emits: %v", shedFilter, err)
		}
	}
}

// The shed HALF must still be validated -- accepting anything before the "#" would let
// a malformed id reach the ::uuid cast in SQL and fail there instead, as a 500.
func TestListQueueStillRejectsNonUUIDShedFilter(t *testing.T) {
	svc, _ := newTestService()
	for _, bad := range []string{"not-a-uuid", "not-a-uuid#3", "#3"} {
		if _, err := svc.ListQueue(context.Background(), ports.ListQueueParams{
			TenantID: testTenant,
			Category: "vaccination_proof",
			ShedID:   bad,
			...
			t.Fatalf("ListQueue(shed_id=%q) should have been rejected", bad)
		}
	}
}
