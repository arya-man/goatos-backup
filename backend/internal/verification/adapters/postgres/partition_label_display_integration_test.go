...
VALUES ($1::uuid, gen_random_uuid(), 'shed', 'Godel 1', 'active') RETURNING location_id::text`, tenantID).Scan(&shedID); err != nil {
	...
	for _, s := range statuses {
		...
		if s.status != string(domain.StatusPending) {
			if _, err := pool.Exec(ctx, `UPDATE verification_items SET status = $3 WHERE tenant_id = $1::uuid AND item_id = $2::uuid`,
				tenantID, item.Item.ItemID, s.status); err != nil {
...
VALUES ($1::uuid, gen_random_uuid(), $2, $3, 'active') RETURNING location_id::text`, tenantID, kind, name).Scan(&id); err != nil {
	...
	if opts.Sheds[0].ID != cbeGandhi {
		t.Fatalf("park-scoped read returned the wrong park's shed: got id %s, want CBE's %s (same name, different park)", opts.Sheds[0].ID, cbeGandhi)
	}
}
