...
func declaredNotificationTypes(t *testing.T) []string {
	...
	for _, entry := range entries {
		...
		for _, match := range singleNotificationTypeConstRe.FindAllStringSubmatch(string(source), -1) {
			seen[match[1]] = true
		}
		for _, match := range notificationTypeConstRe.FindAllStringSubmatch(string(source), -1) {
			seen[match[1]] = true
		}
		for _, match := range notificationTypeLiteralRe.FindAllStringSubmatch(string(source), -1) {
			seen[match[1]] = true
		}
	}
