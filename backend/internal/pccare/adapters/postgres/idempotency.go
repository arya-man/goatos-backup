...
// Request-level idempotency guard for the PC Care write paths (feeddirection idempotency clone).
// Every mutating write reserves its client-supplied idempotency key together with a semantic
// request fingerprint in the SAME transaction as its side effects. An exact replay (same key +
// same fingerprint) returns the original result and reruns no side effects; a same-key /
// different-payload replay is rejected with ports.ErrIdempotencyConflict.

type idemReservation struct {
	// proceed is true on the FIRST claim — run side effects then completeIdempotency. When
	// false the key was already reserved (replay): run NO side effects, return the original
	// result via resultID.
	proceed  bool
	resultID string
	snapshot []byte
...
// requestFingerprint hashes the fields that define the write's effect (order matters).
