// Package goatcreatedrecovery owns the ONE implementation of goat.created gap
// detection and repair. `goat.created` is the sole SM-1 trigger for vaccination
// obligation generation, so a lost event silently leaves a live goat with zero
// vaccination work for an unbounded time (BUG-016). Both entry points share this
// package: the manual repair CLI (backend/cmd/backfill-goat-created) and the
// SCHEDULED reconciliation stage that runs inside the kernel worker
// (backend/internal/kernelstages.GoatCreatedRecoveryStage). Recovery must not
// depend on a human noticing.
package goatcreatedrecovery
...
// Candidate is a live goat with no goat.created identity event.
type Candidate = goatRow

// ScanCandidates returns live goats missing their goat.created event, bounded by
// limit. goatID is optional and pins the scan to one goat.
func ScanCandidates(ctx context.Context, db Querier, tenantID, goatID string, limit int) ([]Candidate, error) {
...
// Recover republishes the missing canonical goat.created event (identity event +
// outbox + audit) for each candidate, in one transaction per goat. Idempotent:
// a goat whose event appeared between scan and repair is skipped.
func Recover(ctx context.Context, db Querier, candidates []Candidate) (int, error) {
	applied := 0
	for _, goat := range candidates {
		ok, err := BackfillOne(ctx, db, goat)
		if err != nil {
			return applied, fmt.Errorf("recover goat %s: %w", goat.GoatID, err)
		}
		if ok {
			applied++
		}
...
// BackfillOne writes the canonical goat.created event for one goat.
