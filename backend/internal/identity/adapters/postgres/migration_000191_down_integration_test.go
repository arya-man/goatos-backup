...
// TestMigration000191DownToleratesIdentityGoatRows is the VACC-REV-13 guard: the DOWN of migration
// 000191 must SUCCEED even after the feature has produced real identity_decisions rows with
...
func TestMigration000191DownToleratesIdentityGoatRows(t *testing.T) {
	...
	// Run the 000191 DOWN section statement-by-statement (it is a NO TRANSACTION migration).
	for _, stmt := range extract000191Down(t) {
		if _, err := pool.Exec(ctx, stmt); err != nil {
			t.Fatalf("000191 down failed with an existing identity_goat row (VACC-REV-13): %v\nstmt: %s", err, stmt)
		}
	}
...
// extract000191Down returns the executable statements of the 000191 DOWN section, dropping goose
// directives and comment lines and splitting on the statement terminator.
func extract000191Down(t *testing.T) []string {
	t.Helper()
	path := filepath.Join(repoRoot(t), "backend", "migrations", "postgres", "000191_goat_identity_correction_decision.sql")
	raw, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read 000191: %v", err)
	}
	...
	if len(stmts) == 0 {
		t.Fatal("no down statements extracted from 000191")
	}
	return stmts
}
