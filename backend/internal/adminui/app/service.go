...
func pageSpecificCopy(id string) map[string]string {
	...
	case "source-load":
		return map[string]string{
			...
			"section.phc_handoffs.note":         "only accepted intake makes an animal eligible for post-arrival Preventive Care (PC)",
			...
			"label.phc_handoff_note":            "Accepting intake is the only handoff that makes a procured animal eligible for post-arrival Preventive Care (PC). It runs once and is idempotent.",
	...
	case "herd-register":
		return map[string]string{
			...
			"note.identifier_required":                 "Both Animal ID 1 and Animal ID 2 are required. ID values are never reused, even after death, sale, transfer, tag loss, or tag breakage.",
			...
			"note.bulk_template":                       "Each row needs Animal ID 1, Animal ID 2, Species, Park, Shed, DOB, Sex, Origin, and Entry date. Bad rows return per-row errors below; they are never silently dropped.",
