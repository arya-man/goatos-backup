...
func navigation() domain.NavigationContract {
	return domain.NavigationContract{
		...
		Groups: []domain.NavigationGroup{
			{
				ID: "phc", Label: "Preventive Care (PC)", Icon: "heart-pulse", DefaultOpen: true, BadgeKey: "phc_open_work",
	...
	return []domain.PageContract{
		...
		page("source-load", "/procurement/source-entry/loads/{load_id}", "/procurement/source-entry/loads/{load_id}", "Source load", "Full source-entry journey timeline, goat rows, decisions, and arrival gate.", "record-drilldown",
			[]domain.TableContract{
				...
				table("phc-handoffs", "Accepted intake → Preventive Care (PC) handoffs", "/procurement/source-entry/loads/{load_id}/phc-handoffs", []string{"goat", "park", "shed", "entry_date", "accepted", "event"}, "phc_handoff"),
			}),
...
func pageSpecificCopy(id string) map[string]string {
	...
	case "calendar":
		return map[string]string{
			...
			"calendar.rhythm.title.phc":               "Preventive Care (PC) vaccination rhythm",
			...
			"calendar.week.title.phc":                 "Preventive Care (PC) this week",
			"calendar.week.scope_only.phc":            "Showing Preventive Care (PC) work only.",
			...
			"calendar.new_event.disabled_reason":      "Calendar events are generated from source-backed obligations. Create a campaign/catch-up via Config or the Preventive Care (PC) catch-up path - not a free-form Calendar entry.",
	...
	case "source-load":
		return map[string]string{
			...
			"section.goats.boundary_note":       "Only accepted intake goats link out to Preventive Care (PC). Rejected-before-truck, arrival-rejected, dead/sold/lost, and unresolved goats stay procurement history and are never shown as Preventive Care (PC) vaccination work.",
			...
			"section.phc_handoffs.title":        "Accepted intake → Preventive Care (PC) handoffs",
			"section.phc_handoffs.note":         "only accepted intake makes a goat eligible for post-arrival Preventive Care (PC)",
			...
			"action.phc_passport":               "Preventive Care (PC) passport",
			...
			"label.procurement_history_no_phc":  "procurement history — no Preventive Care (PC) work",
			...
			"label.phc_handoff_note":            "Accepting intake is the only handoff that makes a procured goat eligible for post-arrival Preventive Care (PC). It runs once and is idempotent.",
			...
			"confirm.accept_intake":             "Accept these goats into the herd and create Preventive Care (PC) handoffs? Only run after arrival reconciliation.",
	...
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc_approval", "require Preventive Care (PC) approval", "", ""),
		...
		},
		{
			...
			Options: []domain.Option{
				option("phc_review", "Preventive Care (PC) review", "missed/lapsed course must be reviewed before restart", "warn"),
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc", "Preventive Care (PC)", "publishable", "ok"),
	...
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc", "Preventive Care (PC)", "Preventive Care (PC)", ""),
