	...
	return []domain.PageContract{
		...
		page("vaccination", "/vaccination", "/vaccination", "Vaccination", "Shed-wise Preventive Care (PC) vaccination: one row per shed with animal-level due/done, planned sessions, capacity, and merged status.", "module-surface",
			[]domain.TableContract{
				...
				table("full-vaccine-schedule", "Full vaccine schedule", "/vaccination/operations", []string{"park", "shed", "type", "animals"}, "schedule_row"),
...
func pageSpecificCopy(id string) map[string]string {
	...
	case "herd-register":
		return map[string]string{
			...
			"section.summary.note":                     "Herd KPIs read the maintained herd-register summary projection; the table pages live goats from /goats/search under the top-bar scope.",
			"section.summary.tooltip":                  "Exact scoped counts from the herd-register summary projection.",
			"section.summary.unavailable":              "Summary projection unavailable",
