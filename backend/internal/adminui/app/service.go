...
func pageSpecificCopy(id string) map[string]string {
	...
	case "sales-economics":
		...
		return map[string]string{
			...
			"section.animals.subtitle": "Every pen with weighed animals, worst daily net first. All figures are per head per day, so pens compare fairly whatever their size. Measured shows how many of the pen's animals these figures are computed from.",
			"section.breeds.title":     "Breed by breed",
			"section.breeds.subtitle":  "What a head of each breed eats per day against what its measured growth returns. Measured shows how many of that breed these figures are computed from — the rest of the herd has not been weighed twice yet. The bar is the feed cost and the line is the return: a bar reaching its line is a breed barely paying for itself, and a red bar past its line is one losing money every day it stays.",
