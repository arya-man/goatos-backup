...
func pageSpecificCopy(id string) map[string]string {
	...
	case "protocol-adherence":
		return map[string]string{
			...
			"gap.verification_pending":  "verification pending",
	...
	case "calendar":
		return map[string]string{
			...
			"calendar.drawer.eligible_goats":          "Eligible goats",
			"calendar.drawer.eligible_goats_empty":    "No goats are linked to this drive yet.",
	...
	case "herd-register":
		return map[string]string{
			...
			"alert.stages.body":                        "A clean vaccination trigger needs a real active animal stage from animal_stage_lookup. Seed or restore stages before registering goats.",
	...
	case "config":
		return map[string]string{
			...
			"drawer.record.note":                                        "Read-only authority record. Editing and publishing happen through the governed draft flow; generated obligations, SOP tasks, and adherence rows come only from published source-backed versions.",
			...
			"modal.rule_editor.placeholder.vaccine_manufacturer":        "manufacturer / source detail",
			...
			"modal.rule_editor.note.feed_dsl":                           "feed_direction stores source tables, parameter families, dimension keys, validation policy, calculation preview outputs, session slots, proof policy, inventory policy, and source approval. Numbers such as 80/20 are source-row examples until reviewed and published.",
...
func configOptionGroups() []domain.OptionGroup {
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				option("ET+TT", "ET+TT", "vaccine_type=toxoid|pathogen_class=bacterial|course_type=booster|disease=Enterotoxaemia + Tetanus|compatibility_group=ET+TT|weeks=4,7|revaccination_days=182|dose_amount=2|vial_doses=100|priority=1|goat_row=true", "goat"),
				option("PPR", "PPR", "vaccine_type=live|pathogen_class=viral|course_type=single|disease=Peste des petits ruminants|compatibility_group=PPR|weeks=16|revaccination_days=1095|dose_amount=1|vial_doses=100|priority=2|goat_row=true", "goat"),
				option("Goat Pox", "Goat Pox", "vaccine_type=live|pathogen_class=viral|course_type=single|disease=Goat Pox|compatibility_group=Goat Pox|weeks=16|revaccination_days=365|dose_amount=1|vial_doses=25|goat_row=true", "goat"),
				option("FMD", "FMD", "vaccine_type=killed|pathogen_class=viral|course_type=single|disease=Foot and mouth disease|compatibility_group=FMD|weeks=12|revaccination_days=274|dose_amount=1|vial_doses=30|goat_row=true", "goat"),
				option("HS", "HS", "vaccine_type=killed|pathogen_class=bacterial|course_type=single|disease=Haemorrhagic septicaemia|compatibility_group=HS|weeks=12|revaccination_days=365|dose_amount=2|vial_doses=100|goat_row=true", "goat"),
				option("Blue Tongue", "Blue Tongue", "vaccine_type=killed|pathogen_class=viral|course_type=booster|disease=Blue Tongue|compatibility_group=Blue Tongue|weeks=16,20|revaccination_days=365|dose_amount=2|vial_doses=100|goat_row=false", "sheep"),
				option("Sheep Pox", "Sheep Pox", "vaccine_type=live|pathogen_class=viral|course_type=single|disease=Sheep Pox|compatibility_group=Sheep Pox|weeks=12|revaccination_days=365|dose_amount=1|vial_doses=100|goat_row=false", "sheep"),
			},
		},
		{
			ID: "dose_units",
		...
		},
		{
			...
			Options: []domain.Option{
				option("phc_review", "PHC review", "missed/lapsed course must be reviewed before restart", "warn"),
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("sex_gender_tag", "sex / gender tag", "Includes F2-Male/F2-Female style tag contamination that must be resolved.", ""),
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("alias_reference_match", "alias/reference match", "Unknown breed, tag, age/stage, sex/gender, feed item, or unit blocks.", ""),
	...
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("temp_field_id", "Temp field ID", "", ""),
				option("park", "Park", "", ""),
				option("shed", "Shed", "", ""),
				option("breed", "Breed", "", ""),
				option("sex", "Sex", "", ""),
