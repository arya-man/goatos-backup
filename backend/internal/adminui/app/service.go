...
func pages() []domain.PageContract {
	return []domain.PageContract{
		...
		page("source-load", "/procurement/source-entry/loads/{load_id}", "/procurement/source-entry/loads/{load_id}", "Source load", "Full source-entry journey timeline, goat rows, decisions, and arrival gate.", "record-drilldown",
			[]domain.TableContract{
				table("load-goats", "Goats in load", "/procurement/source-entry/loads/{load_id}/goats", []string{"goat_source_tag", "selection", "current_stage", "identity", "ownership", "health", "warmup", "downstream"}, "load_goat"),
				...
				table("transit-handoffs", "Transit handoffs", "/procurement/source-entry/loads/{load_id}/transit", []string{"loaded", "dispatched", "status", "discrepancy"}, "transit"),
				...
				table("phc-handoffs", "Accepted intake → PHC handoffs", "/procurement/source-entry/loads/{load_id}/phc-handoffs", []string{"goat", "park", "shed", "entry_date", "accepted", "event"}, "phc_handoff"),
			}),
		...
		page("config", "/config", "/config", "Config — Protocol Rules", "Admin/Data Ops authority for source-backed protocol rules.", "authority-screen",
			[]domain.TableContract{table("protocol-rules", "Protocol rules", "/protocols", []string{"category", "version", "scope", "status", "effective", "linked_sop", "last_publisher", "actions"}, "protocol_id")}),
...
func pageSpecificCopy(id string) map[string]string {
	...
	case "control-tower":
		return map[string]string{
			...
			"link.vaccination_ops":             "PHC · Vaccination ops →",
			...
			"empty.open_gaps_detail":           "No open gaps — every vaccination obligation is on track, or none has been generated yet. Publish a source-backed protocol in Config and a vaccination SOP to start generating obligations.",
	...
	case "action-center":
		return map[string]string{
			...
			"empty.work_board_detail":             "No vaccination obligations yet — the board fills once a source-backed protocol is published and a vaccination SOP exists. Columns below show the work-state shell.",
	...
	case "protocol-adherence":
		return map[string]string{
			...
			"filter.search_reason":      "Search expected, actual, owner, next action, evidence...",
			"filter.reason":             "Use visible-row search and severity chips; backend severity filters stay in the chip row.",
			"filter.search_label":       "Search adherence rows",
			"filter.rows_suffix":        "expected, actual, gap, owner, evidence",
	...
	case "workflows":
		return map[string]string{
			...
			"action.capture_vaccination_proof": "Capture vaccination proof",
			"action.verify_vaccination_proof":  "Verify vaccination proof",
			"empty.catalog":                    "No live workflows yet. A workflow starts when a published protocol generates an obligation — publish a source-backed rule in Config and a vaccination SOP.",
	...
	case "calendar":
		return map[string]string{
			...
			"calendar.rhythm.title.phc":               "PHC vaccination rhythm",
			"calendar.rhythm.note.phc":                "plan drives - prepare teams - execute proof",
			...
			"calendar.rhythm.note.admin_data_ops":     "source review, config approval, import, audit follow-up",
			"calendar.week.title.all":                 "This week",
			"calendar.week.scope_only.all":            "",
			"calendar.week.title.phc":                 "PHC this week",
			"calendar.week.scope_only.phc":            "Showing PHC work only.",
			...
			"calendar.month.cell_note":                "Each cell shows that day's source-backed vaccination due work. Tap an event for its rich detail.",
			"calendar.new_event.label":                "New event",
			"calendar.new_event.disabled_reason":      "Calendar events are generated from source-backed obligations. Create a campaign/catch-up via Config or the PHC catch-up path - not a free-form Calendar entry.",
			"calendar.empty.ok":                       "No vaccination due work for this scope and date window. Events appear once source-backed obligations, drives, boosters, proof/rework, or the stock/config tasks that gate them are due.",
			...
			"label.source_backed_rule":                "Source-backed rule",
			"label.source_backed":                     "source-backed",
			"label.not_source_backed":                 "not source-backed",
	...
	case "vaccination":
		return map[string]string{
			...
			"section.status_matrix.empty_body":            "Columns are the published vaccination protocols; rows are park/shed cohorts. Publish a source-backed protocol in Config and a vaccination SOP — obligations then generate against cohorts and fill this grid.",
			...
			"section.status_matrix.row_hint":              "click a cell → record / verify",
			...
			"section.cohort_detail.row_hint":              "click a row → cohort record",
			...
			"section.supplier_warmup.note":                "Purchased goats start at the supplier / holding farm. HF doses import as completion evidence so accepted-intake goats do not double-dose on arrival. Source Entry owns the write actions; PHC reads the evidence here.",
			"section.supplier_warmup.lifecycle":           "Lifecycle: purchase → supplier warmup (tag + vaccinate · rejectable) → load → arrival → accepted intake → PHC obligations. Rejected-before-truck goats never enter park count or active vaccination work.",
			...
			"drawer.record_verify.title":                  "Record / verify vaccination",
			"drawer.record_verify.aria":                   "Record / verify vaccination",
			"drawer.record_verify.close_label":            "Close record / verify drawer",
			"drawer.record_verify.eyebrow":                "VACCINE",
			"drawer.record_verify.note":                   "Pick vaccine + batch (FEFO), confirm cold-chain, capture proof. Verify posts a consume movement and advances the cohort; overdue escalates to the Health Director.",
			"drawer.record_verify.record_reason":          "Recording a dose is the field worker's vaccination.execute SOP action (scan → administer → record dose → upload shed + vial video). This command surface has no record route — /vaccination/operations + /vaccination/execution return status + counts only, with no task_id for POST /app/tasks/{task_id}/submissions or /app/proofs/*.",
			...
			"drawer.warmup.note":                          "Source Entry owns Holding-Farm write actions. PHC reads HF dose evidence here so arrival vaccination never double-doses.",
			...
			"drawer.shed_event.eyebrow":                   "RECORD",
			...
			"drawer.import.new_drives_body":               "Publish a source-backed protocol rule in Config → obligations generate → the sweeper batches a shed drive.",
			...
			"drawer.new_drive.publish_rule_body":          "Config → vaccination category → publish a source-backed protocol version.",
			...
			"drawer.sop.eyebrow":                          "PHC · VACCINATION",
	...
	case "shed-execution":
		return map[string]string{
			"crumb":                      "PHC · Vaccination · Execution",
	...
	case "source-entry":
		return map[string]string{
			...
			"drawer.load.note":               "This load starts before park arrival: purchase/source → holding warmup → HF vaccination evidence → health selection → pre-dispatch. Accepted-intake goats then feed the PHC vaccination flow.",
			...
			"action.source_goat_added":       "Source goat added to load.",
			...
			"action.accept_intake_recorded":  "Accepted intake recorded; PHC handoffs created when eligible.",
	...
	case "source-load":
		return map[string]string{
			...
			"section.goats.title":               "Goats in load",
			"section.goats.note":                "per-goat journey state — not load totals only",
			"section.goats.boundary_note":       "Only accepted intake goats link out to PHC. Rejected-before-truck, arrival-rejected, dead/sold/lost, and unresolved goats stay procurement history and are never shown as PHC vaccination work.",
			...
			"section.arrival_gate.empty":        "No arrival review yet. The arrival gate reconciles expected vs arrived goats (matched / missing / extra, health and weight flags) at the park before any goat becomes clean herd truth.",
			"section.transit.title":             "Transit handoffs",
			"section.holding.title":             "Holding stays",
			"section.holding.note":              "breeding 45–70d · fattening/non-breeding 0–14d",
			"section.source_health.title":       "Source health checks",
			"section.phc_handoffs.title":        "Accepted intake → PHC handoffs",
			"section.phc_handoffs.note":         "only accepted intake makes a goat eligible for post-arrival PHC",
			"section.actions.title":             "Source-entry actions",
			"section.actions.note":              "add goat · source health · pre-dispatch · dispatch · arrival · accept intake",
			"action.back_source_entry":          "Back to Source Entry",
			"action.phc_passport":               "PHC passport",
			"empty.timeline":                    "No journey events recorded for this load yet.",
			"empty.load_goats":                  "No goats added to this load yet.",
			"label.event_fallback":              "event",
			"label.goat_prefix":                 "goat",
			"label.arrival_prefix":              "arrival",
			"label.park_prefix":                 "park",
			"label.matched":                     "matched",
			"label.missing":                     "missing",
			"label.extra_unknown":               "extra/unknown",
			"label.reviewed":                    "reviewed",
			"label.expected":                    "expected",
			"label.load":                        "Load",
			"label.purchase":                    "purchase",
			"label.planned_dispatch":            "planned dispatch",
			"label.procurement_history_no_phc":  "procurement history — no PHC work",
			...
			"label.phc_handoff_note":            "Accepting intake is the only handoff that makes a procured goat eligible for post-arrival PHC. It runs once and is idempotent.",
			"form.add_goat.title":               "Add source goat",
			...
			"field.to_location_id":              "To location id (required)",
			"field.from_location_id":            "From location id",
			...
			"action.add_source_goat":            "Add source goat",
			...
			"empty.add_goats_first":             "Add source goats first. HF dose evidence must be keyed to a goat in this procurement load.",
			"empty.hf_evidence":                 "No HF vaccination evidence imported for this load yet.",
			"empty.pre_dispatch":                "No goats currently awaiting source health or a pre-dispatch decision. Accepted-intake and terminal goats are not shown here.",
			"note.source_goat_identity":         "A source goat is procurement-only identity — not clean park/herd truth until accepted intake.",
			"note.arrival_rows":                 "Only goats listed as accepted advance to arrival-accepted and become eligible for intake.",
			"confirm.pre_dispatch.prefix":       "Record pre-dispatch decision for",
			"confirm.pre_dispatch.suffix":       "Rejection before truck keeps the goat in procurement history (no PHC work).",
			"confirm.accept_intake":             "Accept these goats into the herd and create PHC handoffs? Only run after arrival reconciliation.",
	...
	case "herd-register":
		return map[string]string{
			...
			"filter.herd_search_label":                 "Search (RFID / old tag / id)",
			...
			"action.register_goat":                     "Register goat",
			"action.import_sheet":                      "Import sheet",
			...
			"action.goat_registered_identity_review":   "Goat registered; identity needs review before vaccination work is generated.",
			"action.goat_registered_ineligible":        "Goat registered; no vaccination obligations generated because the goat is ineligible by published rules.",
			"action.goat_registered_no_generation":     "Goat registered; no vaccination generation applicable.",
			"reason.report_pending":                    "No herd report API exists in this slice. New report stays disabled.",
			"drawer.register.title":                    "Register goat",
			"drawer.register.subtitle":                 "Creates one canonical goat → emits goat.created → vaccination obligations generate.",
			"drawer.import.title":                      "Import sheet",
			"drawer.import.subtitle":                   "Bulk register goats — download template, paste/upload CSV, preview, then commit.",
			"drawer.passport.aria":                     "Goat Passport",
			"drawer.passport.close_label":              "Close Goat Passport drawer",
			"alert.locations.title":                    "Locations unavailable",
			"alert.locations.body":                     "A clean create needs a real park and vaccination-usable shed from the locations master. Configure locations (or check the backend) before registering — no goat is created without a valid park/shed.",
			"empty.herd":                               "No goats for this scope yet. Use Register goat or Import sheet to add the first goats — each emits goat.created and generates vaccination obligations.",
			"empty.herd_filtered":                      "No goats match these filters for this scope.",
			"empty.unavailable":                        "Herd is unavailable until the goats read API responds.",
			"field.rfid":                               "RFID",
			"field.old_tag":                            "Old / ear tag",
			"field.temp_field_id":                      "Temporary field id",
			...
			"placeholder.rfid":                         "e.g. RF-9001",
			"placeholder.old_tag":                      "e.g. CB-201",
			...
			"note.identifier_required":                 "At least one identifier is required — RFID, old tag, or a temporary field id. A duplicate active RFID returns a conflict for review, not a silent merge.",
			...
			"note.bulk_template":                       "Each row needs at least one identifier (RFID or Old tag) plus Park and Shed (codes). Sex = female / male / unknown. Origin = birth / procured / imported / unknown. Bad rows return per-row errors below; they are never silently dropped.",
			...
			"label.identity_review_rows":               "identity review rows",
	...
	case "config":
		return map[string]string{
			...
			"section.rules.note":                               "source-backed config list",
			...
			"empty.rules":                                      "No protocol rules yet — the engine stands up empty. Author one with New draft rule; only source-backed, approved versions can publish and generate obligations.",
			...
			"modal.rule_editor.field.feed_stage":               "Feed Direction - animal stage / class",
			"modal.rule_editor.field.ration":                   "Ration / feed item - quantity",
			"modal.rule_editor.field.session_timing":           "Session timing",
			"modal.rule_editor.placeholder.session_timing":     "09:00,15:00",
			"modal.rule_editor.field.proof":                    "Packing / execution proof",
			"modal.rule_editor.placeholder.packing_proof":      "pack_qty,feed_item,lot,video",
			"modal.rule_editor.placeholder.execution_proof":    "distribution_video,consumed_qty,water_check",
			"modal.rule_editor.field.inventory":                "Inventory reserve / consume / release",
			...
			"modal.rule_editor.label.individual_override":      "allow individual override / catch-up",
			...
			"modal.rule_editor.field.source_review":            "Source & review (publish needs a real source + approval)",
			"modal.rule_editor.placeholder.source_ref":         "source_ref (sheet / file id)",
			"modal.rule_editor.placeholder.reviewed_by":        "reviewed_by",
			"modal.rule_editor.placeholder.approved_by":        "approved_by",
			"modal.rule_editor.placeholder.approved_at":        "approved_at RFC3339 from source",
			"modal.rule_editor.label.rule_dsl":                 "Stored as rule_dsl (JSONB)",
			"modal.rule_editor.rule_dsl_aria":                  "rule_dsl preview",
			"modal.rule_editor.note.feed_dsl":                  "Feed Direction uses ration + session_timing + inventory_policy; no vaccination dose/lot fields are shown. Source is nested under source. JSON-Schema-validated · versioned · not YAML.",
			"modal.rule_editor.note.vaccination_dsl":           "schedule = ARRAY of dose rows (multi-dose / multi-phase). Canonical keys: repeat_until_after_age, proof_policy. Next due from last accepted completion (not DOB) when prior history exists. Source nested under source. JSON-Schema-validated · versioned · not YAML.",
			"modal.rule_editor.table.feed_title":               "Feed Direction rule rows",
			"modal.rule_editor.table.session":                  "Session",
			"modal.rule_editor.table.feed_item":                "Feed item",
			"modal.rule_editor.table.quantity":                 "Quantity",
			"modal.rule_editor.table.proof":                    "Proof",
			"modal.rule_editor.table.inventory":                "Inventory policy",
			"modal.rule_editor.table.packing_execution_proof":  "packing + execution proof",
			...
			"modal.rule_editor.table.proof_policy":             "proof_policy",
			...
			"modal.rule_editor.impact_title_feed":              "Impact preview - computed from ration/session rules",
			"modal.rule_editor.preview_feed_pending":           "backend preview lands with the feed_direction domain",
			...
			"modal.rule_editor.preview_empty_feed":             "Feed preview will compute eligible sheds/classes, session obligations, packing batches, and reserve/consume/release inventory once the feed domain endpoint lands.",
			"modal.rule_editor.preview_empty_vaccination":      "Run a preview to compute live eligible goats, obligations, drive batches, and doses-vs-stock from the schedule.",
	...
	case "sops":
		return map[string]string{
			...
			"filter.domain.current":                   "Current visible SOP slice is PHC / Vaccination",
			...
			"modal.builder.eyebrow":                   "SOP · PHC / VACCINATION",
			...
			"modal.builder.domain_aria":               "Domain — locked to PHC / Vaccination",
			"modal.builder.domain_title":              "Domain is locked to PHC / Vaccination for the current slice",
			"modal.builder.domain_label":              "PHC / Vaccination",
	...
	case "goat-passport":
		return map[string]string{
			...
			"vaccination.empty_history":      "No vaccination history yet. Once a source-backed protocol is published and a dose is administered + verified, it appears here with its proof/verification status and the source protocol version.",
			...
			"label.goat_id":                  "Goat ID",
			"label.rfid":                     "RFID",
			"label.primary_old_tag":          "Primary old tag",
	...
	case "workflows", "workflow-record":
		return append(withGenericOptionGroups([]domain.OptionGroup{{
			...
			Options: []domain.Option{
				option("config", "Config rule published", "source-backed protocol version goes live", "ok"),
		...
		return withGenericOptionGroups([]domain.OptionGroup{
			{
				ID: "identifier_types",
				Options: []domain.Option{
					option("old_tag", "old_tag", "", ""),
					option("rfid", "rfid", "", ""),
					option("visual_tag", "visual_tag", "", ""),
					option("sheet_row_id", "sheet_row_id", "", ""),
					option("purchase_load_id", "purchase_load_id", "", ""),
					option("temp_field_id", "temp_field_id", "", ""),
					option("external_system_id", "external_system_id", "", ""),
				},
			},
			{
				ID: "evidence_types",
	...
	return []domain.OptionGroup{
		{
			...
			Options: []domain.Option{
				...
				option("feed_direction", "feed_direction", "", ""),
			},
		},
		{
			ID: "rule_scopes",
		...
		},
		{
			...
			Options: []domain.Option{
				option("vaccination.code", "vaccination.enterotox", "", ""),
				option("vaccination.name", "Enterotoxaemia", "", ""),
				option("feed_direction.code", "feed_direction.daily_ration", "", ""),
				option("feed_direction.name", "Daily ration - stage plan", "", ""),
				option("deworming.code", "deworming.famacha", "", ""),
				option("deworming.name", "FAMACHA-triggered deworming", "", ""),
			},
		},
		{
		...
		},
		{
			...
			Options: []domain.Option{
				option("any", "any", "", ""),
				option("exclude_pregnant", "exclude pregnant", "", ""),
				option("exclude_lactating", "exclude lactating", "", ""),
				option("pregnant_only", "pregnant only", "", ""),
			},
		},
		{
			ID: "defer_states",
			Options: []domain.Option{
				...
				option("sick", "sick / under_treatment", "", ""),
			},
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc_approval", "require PHC approval", "", ""),
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("until_age", "until_age", "", ""),
				option("after_age", "after_age", "", ""),
			},
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc_approval", "phc_approval", "", ""),
				option("defer", "defer", "", ""),
			},
		},
		{
			...
			Options: []domain.Option{
				option("vacc-sop v2", "vacc-sop v2", "", ""),
				option("deworm-sop v1", "deworm-sop v1", "", ""),
			},
		},
		{
			ID: "feed_classes",
			Options: []domain.Option{
				option("all", "all", "", ""),
				option("kid", "kid", "", ""),
				option("grower", "grower", "", ""),
				option("fattening", "fattening", "", ""),
				option("pregnant_lactating", "pregnant lactating", "", ""),
				option("buck", "buck", "", ""),
		...
		},
		{
			ID: "feed_items",
			Options: []domain.Option{
				option("Mesha concentrate", "Mesha concentrate", "", ""),
				option("Masoor bhusa", "Masoor bhusa", "", ""),
				option("Toor dhal bhusa", "Toor dhal bhusa", "", ""),
				option("Green feed", "Green feed", "", ""),
				option("Dry feed", "Dry feed", "", ""),
				option("custom", "custom", "", ""),
			},
		},
		{
		...
		},
		{
			...
			Options: []domain.Option{
				option("reserve_consume_release", "reserve before pack -> consume verified qty -> release remainder", "", ""),
				option("consume_on_verify", "consume only after verifier accepts execution proof", "", ""),
				option("manual_hold", "manual reserve hold; release by Data Ops", "", ""),
			},
		},
		{
			...
			Options: []domain.Option{
				...
				option("draft_publishable", "Draft · approved · publishable", "", "info"),
				option("draft_pending_approval", "Draft · source-backed · pending approval", "", "info"),
				option("draft_not_publishable", "Draft · not source-backed · cannot publish", "", "warn"),
			},
		},
	}
}
	...
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("goat_scan", "Scan goat RFID / tag", "", ""),
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("goat_scan", "goat scan/RFID", "", ""),
	...
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("identity_review", "Identity Review Status", "needs review / clean", ""),
...
func calendarOptionGroups() []domain.OptionGroup {
	return []domain.OptionGroup{
		...
		},
		{
			...
			Options: []domain.Option{
				option("vaccination", "Vaccination", "Calendar is currently showing the PHC vaccination module.", ""),
			},
		},
		{
			ID: "calendar_workstream_tabs_phc",
			Options: []domain.Option{
				option("vaccination", "Vaccination", "Calendar is currently showing the PHC vaccination module.", ""),
			},
		},
		...
		},
		{
			...
			Options: []domain.Option{
				...
				option("phc_stock_anti_misuse", "Stock anti-misuse", "", ""),
				option("vaccination_config_source_approval", "Config / source approval", "", ""),
			},
		},
		{
			ID: "calendar_status",
...
func procurementOptionGroups() []domain.OptionGroup {
	return []domain.OptionGroup{
		...
		{
			ID: "proc_identity_review_state",
			Options: []domain.Option{
				option("pending", "Identity: pending", "", "warn"),
				option("clean", "Identity: clean", "", "ok"),
				option("conflict", "Identity: conflict", "", "dng"),
				option("unknown_extra", "Identity: unknown/extra", "", "dng"),
			},
		},
		{
		...
		},
		{
			...
			Options: []domain.Option{
				option("fattening", "0-14d", "fattening / non-breeding warmup can be same-day to about two weeks", ""),
				option("non_breeding", "0-14d", "fattening / non-breeding warmup can be same-day to about two weeks", ""),
				option("breeding", "45-70d", "breeding stock warmup is expected to be 45-70 days", ""),
				option("unspecified", "set purpose", "set purpose to classify the warmup window; fallback is the breeding-safe 70 day ceiling", ""),
			},
		},
		{
...
func humanLabel(key string) string {
	...
	case "animal_stage":
		return "Animal stage"
	case "goat_source_tag":
		return "Goat / source tag"
	case "current_stage":
