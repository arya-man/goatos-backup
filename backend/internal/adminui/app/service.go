...
func pageSpecificCopy(id string) map[string]string {
	...
	case "people":
		...
		return map[string]string{
			...
			"action.person_created_existing":       "Person added. This email already had a login with its own password — that password is unchanged and the standard one does not apply.",
			"action.person_created_password_added": "Person added. This email already signs in with Google — the standard password now also works.",
