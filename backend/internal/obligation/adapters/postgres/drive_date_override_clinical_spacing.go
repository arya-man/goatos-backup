...
SELECT COALESCE(NULLIF(pr.vaccine_code, ''), prd.vaccine_code, $2),
       COALESCE(NULLIF(ceo_ai.vaccine_label_for(pr.dose_code), ''), NULLIF(pr.vaccine_code, ''), prd.vaccine_code, $2),
...
events AS (
  SELECT vc.goat_id,
         ...
         COALESCE(NULLIF(ceo_ai.vaccine_label_for(pr.dose_code), ''), NULLIF(pr.vaccine_code, ''), prd.vaccine_code, '') AS vaccine_label,
