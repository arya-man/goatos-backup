...
// Idempotency guard for the obligation module's REQUEST-level write paths (for example the mobile
// "reschedule obligation" endpoint), as distinct from the module's existing EVENT-level dedup
// (qtx.ReserveIdempotencyKey/CompleteIdempotencyKey used elsewhere in repository.go to guard individual
// obligation_status_events rows by a deterministic, server-derived event key). This file mirrors the
// procurement module's proven request-level guard
// (backend/internal/procurement/adapters/postgres/idempotency.go) exactly: every mutating write reserves
...
// primary key cannot collide across tenants, nor across different obligation operations that happen to
