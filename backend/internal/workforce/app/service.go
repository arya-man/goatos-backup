...
func (s *Service) Bootstrap(ctx context.Context, tenantID, actorID, deviceID, traceID string) (*domain.BootstrapResponse, error) {
	...
	return &domain.BootstrapResponse{
		...
		FeatureFlags: map[string]bool{
			...
			"rfid_scan":     hasCapability(caps, "rfid.scan"),
		},
		VisibleNavigation:       navigationFor(caps),
		TaskQueueDescriptors:    queuesFor(caps),
		PinnedSOPVersions:       []domain.BootstrapSOPVersion{},
		SupportedFieldTypes:     []string{"text", "number", "date_time", "boolean", "select", "multiselect", "goat_scan", "rfid_scan", "goat_lookup", "shed_picker", "photo_proof", "video_proof"},
...
func validRoleHint(value string) bool {
	switch value {
	case "operator", "park_head", "verifier", "supervisor", "admin", "other":
		return true
	default:
		return false
	}
...
func validRole(value string) bool {
	switch value {
	case "admin", "park_head", "operator", "verifier", "ceo_internal":
		return true
	default:
		return false
	}
