...
func TestProfileEntryPlacementFollowsModuleCount(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			// (leadershipModuleKeys), so a park head holds three modules and gets the drawer
			// like any other >=2-module principal. There is no park-head exception.
			name:         "park head holds the preventive-care union",
