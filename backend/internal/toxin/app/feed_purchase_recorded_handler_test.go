...
// The event registry's proof for procurement.feed_purchase.recorded → toxin: the
...
// inserting an orphan.
func TestFeedPurchaseRecordedCreatesTheLoadsTask(t *testing.T) {
	repo := &creatingRepo{}
	bus := eventbus.NewInProcessBus()
	NewFeedPurchaseRecordedHandler(repo, nil).Register(bus)
	...
	err := bus.Publish(context.Background(), eventbus.Event{
		ID:       "evt-1",
		Type:     "procurement.feed_purchase.recorded",
	...
	if got.FeedPurchaseID != "11111111-0000-4000-8000-000000000001" ||
		...
		got.PurchaseDate != "2026-08-25" || got.QuantityKg != 2000 ||
	...
	if err := bus.Publish(context.Background(), eventbus.Event{
		ID: "evt-2", Type: "procurement.feed_purchase.recorded", TenantID: "t",
