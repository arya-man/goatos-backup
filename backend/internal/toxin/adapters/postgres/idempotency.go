...
// Idempotency guard for toxin write paths — the same shared idempotency_keys contract the
// procurement adapter uses: reserve key + semantic fingerprint in the SAME transaction as
