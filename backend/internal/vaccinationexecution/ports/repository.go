...
type Repository interface {
	...
	// ShedOwnershipReader port, not by this query. Served exclusively from the vaccination-shed read
	// model (indexed projection lookup, C35-002) -- returns domain.ErrProjectionUnavailable rather than
	// a compute-on-read fallback when the tenant has no serving projection version yet.
