...
// listAlertsSQL reads the vaccination lifecycle notifications already routed to
// one person. It is the vaccination twin of the weighing feed and reads the SAME
// shared plumbing table, differing only in the module discriminator.
//
// MODULE DISCRIMINATOR: context->>'message_key' LIKE 'vaccination.%'. Every
// vaccination producer stamps it -- "vaccination.record.closed",
// "vaccination.proof.rework", "vaccination.proof.approved",
// "vaccination.proof.pending.verifier", "vaccination.proof.pending.leadership".
// Weighing rows carry "weighing.*" and are simply not selected, so the two module
// feeds cannot bleed into each other.
//
// IDENTITY: context->>'member_id' is the canonical workforce_member_id the
// consumer resolved BEFORE writing, so filtering on it IS the audience check. An
// operator can never see another operator's alert here because they were never a
// recipient of it. This is the reason the feed needs no roster or scope join.
//
...
// predicate keep this off a full-history scan.
//
// scale-guard:ignore: workforce-scale read -- one person's own vaccination notifications inside a fixed 30-day window, keyset-paginated. Never herd-scale: notification_requests rows are produced per work-state transition per recipient device, not per animal.
const listAlertsSQL = `
WITH me AS (
  SELECT COALESCE(
...
mine AS (
  ...
  WHERE nr.tenant_id = $1::uuid
    ...
    AND nr.context->>'message_key' LIKE 'vaccination.%'
...
SELECT
  ...
  COALESCE(mine.context->>'drive_id', ''),
...
func (r *Repository) ListAlerts(
	...
	if err != nil {
		return domain.AlertPage{}, fmt.Errorf("vaccination: list alerts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&alert.ShedLabel, &alert.DriveID, &alert.ParkID,
		); err != nil {
			return domain.AlertPage{}, fmt.Errorf("vaccination: scan alert: %w", err)
		}
		...
		if strings.TrimSpace(alert.Target) == "" {
			alert.Target = "/vaccination"
		}
		page.Items = append(page.Items, alert)
	}
	if err := rows.Err(); err != nil {
		return domain.AlertPage{}, fmt.Errorf("vaccination: list alerts rows: %w", err)
	}
