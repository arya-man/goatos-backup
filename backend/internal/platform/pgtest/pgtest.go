...
// dropTestDatabase closes the test pool, terminates any residual sessions on the clone, and drops
// it. Cleanup failures are surfaced via t.Error without masking the original test failure.
func dropTestDatabase(t *testing.T, pool, admin *pgxpool.Pool, db string) {
	...
	// Terminate leaked sessions (goroutines a test forgot to close) so DROP DATABASE does not fail.
	if _, err := admin.Exec(ctx,
		`SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = $1 AND pid <> pg_backend_pid()`,
		db); err != nil {
		t.Errorf("pgtest: terminate sessions for %s: %v", db, err)
	}
	...
	t.Errorf("pgtest: drop database %s: %v", db, dropErr)
}
