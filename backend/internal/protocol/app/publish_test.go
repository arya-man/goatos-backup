...
func TestValidateExecutionContract(t *testing.T) {
	...
	missingReproductiveExclusions.RuleDsl = []byte(`{"vaccine":{"code":"ET+TT","name":"ET+TT","type":"killed"},"eligibility":{"animal_stage":"K1","sex":"all","breed":"all","lifecycle":"alive","health":"any","reproductive":"any","defer_states":[]},"schedule":[{"dose_code":"primary","trigger_type":"birth_age","dose_amount":2,"dose_unit":"ml","route_site":"subcutaneous","due_window_days":7,"max_delay_days":7,"course_lapse_policy":"pc_review"}]}`)
...
func TestValidateRuleDSLRejectsUnknownKeys(t *testing.T) {
	valid := []byte(`{"category":"vaccination","scope":{"type":"tenant","id":null},"vaccine":{"code":"ET+TT","name":"ET+TT","type":"killed"},"eligibility":{"animal_stage":"K1","defer_states":["icu"]},"schedule":[{"dose_code":"primary","sop_label":"display","dose_amount":2,"dose_unit":"ml","route_site":"subcutaneous","max_delay_days":7,"course_lapse_policy":"pc_review"}]}`)
...
func TestPublishVersionDelegatesDraftPublishToRepository(t *testing.T) {
	...
	if string(repo.createdRule.EligibilityJSON) != `{"animal_stage":"K1","sex":"all","breed":"all","lifecycle":"alive","health":"any","reproductive":"any","exclude_reproductive_states":["pregnant","lactating"],"defer_states":["sick","under_treatment","icu","quarantine"]}` {
...
func TestPublishVersionMaterializesMatrixRowMetadata(t *testing.T) {
	...
	if meta.Vaccine.Code != "SHEEP_POX" || meta.Vaccine.PathogenClass != "viral" {
...
func TestPublishVersionMatrixInheritsVersionEligibilityAndCanonicalizesAny(t *testing.T) {
	...
	dsl["eligibility"] = map[string]any{
		...
		"defer_states":                []any{"sick", "under_treatment", "icu", "quarantine"},
	}
