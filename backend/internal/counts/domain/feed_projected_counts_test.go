...
func TestFeedShiftingLeadDays(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "emergency moves fast so feed follows one day behind", priority: "emergency", want: 1},
		{name: "normal takes the standard two day lead", priority: "normal", want: 2},
		// 'high' is a QUEUE-ORDER signal, not an execution-speed signal. Treating it as an
		// emergency would pull animals into the destination ration a day early.
		{name: "high is queue order not speed so it stays at two days", priority: "high", want: 2},
		{name: "priority is matched case insensitively", priority: "EMERGENCY", want: 1},
		{name: "surrounding whitespace does not change the lead", priority: "  emergency  ", want: 1},
...
func TestFeedEffectiveBusinessDate(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "emergency approved on the 10th is feed effective on the 11th",
			approvedAt: ist(2026, time.July, 10, 9),
			priority:   "emergency",
			want:       day(2026, time.July, 11),
		},
		{
			name:       "normal approved on the 10th is feed effective on the 12th",
			approvedAt: ist(2026, time.July, 10, 9),
			priority:   "normal",
...
func TestFeedShiftingCountsToward(t *testing.T) {
	...
	cases := []struct {
		...
		// --- emergency: approval day X, effective X+1 ---
		{
			name:       "emergency does not count on the approval day itself",
		...
		{
			name:       "emergency counts on the day after approval",
			approvedAt: approvedOn10th,
			priority:   "emergency",
		...
		// --- normal: approval day X, effective X+2 ---
		{
			name:       "normal does not count on the approval day itself",
			approvedAt: approvedOn10th,
			priority:   "normal",
			target:     day(2026, time.July, 10),
			want:       false,
		...
		{
			name:       "normal does not count one day after approval",
			approvedAt: approvedOn10th,
			priority:   "normal",
			target:     day(2026, time.July, 11),
			want:       false,
		...
		{
			name:       "normal counts two days after approval",
			approvedAt: approvedOn10th,
			priority:   "normal",
		...
		{
			name:       "high follows the normal two day lead",
			approvedAt: approvedOn10th,
			priority:   "high",
			target:     day(2026, time.July, 11),
			want:       false,
		...
		{
			name:       "high counts two days after approval",
			approvedAt: approvedOn10th,
			priority:   "high",
			target:     day(2026, time.July, 12),
			want:       true,
		...
		{
			name:       "an overdue emergency still counts five days after approval",
			approvedAt: approvedOn10th,
			priority:   "emergency",
		...
		{
			name:       "an overdue normal still counts five days after approval",
			approvedAt: approvedOn10th,
			priority:   "normal",
			target:     day(2026, time.July, 15),
			want:       true,
...
func TestFeedShiftingIsOverdue(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:       "normal effective exactly on the target day is not overdue",
			approvedAt: approvedOn10th,
			priority:   "normal",
			target:     day(2026, time.July, 12),
			want:       false,
		...
		{
			name:       "normal five days past approval is overdue",
			approvedAt: approvedOn10th,
			priority:   "normal",
			target:     day(2026, time.July, 15),
			want:       true,
...
func TestFeedShiftingOverdueImpliesCounting(t *testing.T) {
	...
	approvedAt := ist(2026, time.July, 10, 9)
	for _, priority := range []string{"emergency", "normal", "high", "unknown"} {
