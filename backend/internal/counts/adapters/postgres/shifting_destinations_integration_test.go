...
// TestShiftingDestinationCatalogPartitionDisplayFormat verifies the canonical display format is used for partitions.
// OL-2: Six identical "Godel 1" rows (partition dropped) - FIX: return distinct display labels
// OL-3: "Godel 1 1" instead of "Godel 1 - 1" (naive space join) - FIX: use canonical Display() with dash separator
func TestShiftingDestinationCatalogPartitionDisplayFormat(t *testing.T) {
	ctx := context.Background()
	pool := setupCountsDB(t, ctx)
	seedCustodianParty(t, ctx, pool)
	repo := NewRepository(pool, 10*time.Second)
	...
	const (
		partitionedShedID = "00000000-0000-4000-8000-000000004201"
		bareShediD        = "00000000-0000-4000-8000-000000004202"
	)

	// Seed a partitioned shed with 3 partitions (1, 2, 10) and a bare shed
	if _, err := pool.Exec(ctx, `
INSERT INTO locations (location_id, tenant_id, location_type, location_code, name, parent_location_id, status)
VALUES
  ($2::uuid, $1::uuid, 'shed', 'GODEL1',   'Godel 1', $3::uuid, 'active'),
  ($4::uuid, $1::uuid, 'shed', 'BARE',     'Yashoda', $3::uuid, 'active')
ON CONFLICT (location_id) DO NOTHING`,
		countsTenant, partitionedShedID, countsPark, bareShediD); err != nil {
		t.Fatalf("seed sheds: %v", err)
	}

	// Seed the shed_partitions catalog for the partitioned shed
	if _, err := pool.Exec(ctx, `
...
VALUES
  ...
  ($1::uuid, $2::uuid, '10', '10', 'active', 'manual')
ON CONFLICT (tenant_id, shed_id, normalized_label) DO NOTHING`,
		countsTenant, partitionedShedID); err != nil {
	...
	// Find the partitioned shed in the catalog
	var godel1Sheds []struct {
		display    string
		partLabel  *string
		shedID     string
	}
	var bareSheds []struct {
		display   string
		shedID    string
	}
	...
	for _, park := range catalog.Parks {
		for _, shed := range park.Sheds {
			if shed.ShedID == partitionedShedID {
				godel1Sheds = append(godel1Sheds, struct {
					display    string
					partLabel  *string
					shedID     string
				}{display: shed.Display, partLabel: shed.PartitionLabel, shedID: shed.ShedID})
			}
			if shed.ShedID == bareShediD {
				bareSheds = append(bareSheds, struct {
					display   string
					shedID    string
				}{display: shed.Display, shedID: shed.ShedID})
			}
		}
	}

	// Verify we have 3 distinct partitions of Godel 1 (fixes OL-2: six duplicates)
	if len(godel1Sheds) != 3 {
		t.Fatalf("Godel 1 appears %d times, want exactly 3 partitions", len(godel1Sheds))
	}

	// Verify each partition has a distinct, correctly formatted display label (fixes OL-3: "Godel 1 1")
	displaysSeen := map[string]bool{}
	for _, row := range godel1Sheds {
		// Verify the display matches canonical format: "Shed Name - PartitionLabel" with dash
		if row.display != "Godel 1 - 1" && row.display != "Godel 1 - 2" && row.display != "Godel 1 - 10" {
			t.Fatalf("Display=%q, want one of: Godel 1 - 1, Godel 1 - 2, Godel 1 - 10", row.display)
		}
		displaysSeen[row.display] = true
	}

	// Verify all 3 displays are unique (OL-2 fix)
	if len(displaysSeen) != 3 {
		t.Fatalf("Godel 1 partitions have %d unique displays, want 3. Displays: %v", len(displaysSeen), displaysSeen)
	}

	// Verify the bare shed has no partition and renders just "Yashoda"
	if len(bareSheds) != 1 {
		t.Fatalf("bare shed appears %d times, want exactly 1", len(bareSheds))
	}
	if bareSheds[0].display != "Yashoda" {
		t.Fatalf("bare shed Display=%q, want just %q (no partition suffix)", bareSheds[0].display, "Yashoda")
	}
}
