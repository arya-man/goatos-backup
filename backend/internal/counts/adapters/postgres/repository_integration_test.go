...
func insertHerdGoat(t *testing.T, ctx context.Context, pool *pgxpool.Pool, goatID, displayID, sex, breed, lifecycle, ageBand string) {
	t.Helper()
	if _, err := pool.Exec(ctx, `
INSERT INTO goats (
  goat_id, tenant_id, display_id, species, breed, sex, lifecycle_status,
  age_band, custodian_party_id, current_location_id, park_id
) VALUES (
  $1::uuid, $2::uuid, $3, 'goat', $4, $5, $6, $7,
  '00000000-0000-4000-8000-000000001001'::uuid, $8::uuid, $9::uuid
)`,
		goatID, countsTenant, displayID, breed, sex, lifecycle, ageBand, countsShedA, countsPark); err != nil {
		t.Fatalf("seed herd goat %s: %v", displayID, err)
	}
}
...
func shiftingEventWithBreedStage(logicalKey, idemKey, payloadHash, fp, breedKey, breedLabel, stageValue string) domain.ShiftingEvent {
	...
	return domain.ShiftingEvent{
		TenantID: countsTenant, LogicalShiftingEventKey: logicalKey, Priority: "high", Category: "breeding",
