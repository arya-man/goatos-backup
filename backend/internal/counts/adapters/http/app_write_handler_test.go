...
// TestRecordShiftingEventRejectsOmittedImpactsWhenNotExactlyOneAnimal pins the boundary of the
// relaxation. Zero animals has nothing to derive from; two or more would require the server to
// invent a cohort split and a pregnancy/lactation/warm-up distribution nobody entered.
func TestRecordShiftingEventRejectsOmittedImpactsWhenNotExactlyOneAnimal(t *testing.T) {
	otherGoatID := "99999999-9999-4999-8999-999999999999"
	cases := map[string]struct {
		body     map[string]any
		wantCode string
	}{
		// No goat_ids at all: the pre-existing goat_ids requirement still fires first, and its
		// message is the more actionable one for an operator.
		"zero animals": {body: shiftingBodyNoImpacts(), wantCode: "missing_goat_ids"},
		"two animals":  {body: shiftingBodyNoImpacts(testGoatID, otherGoatID), wantCode: "missing_impacts"},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			repo.goatFacts = map[string]domain.GoatShiftingFact{
				testGoatID:  {GoatID: testGoatID, BreedKey: "sirohi", BreedLabel: "Sirohi"},
				otherGoatID: {GoatID: otherGoatID, BreedKey: "sirohi", BreedLabel: "Sirohi"},
			}
			approvals := newFakeApprovalWorkflow()
			mux := newTestServer(t, countsapp.NewService(repo), approvals, newFakeGoatValidator())
