...
import (
	...
	"github.com/vgoats/goatos/backend/internal/counts/app"
...
func Register(mux *http.ServeMux, h *Handler) {
	mux.HandleFunc("GET /analytics/counts/dashboard", h.GetDashboard)
	mux.HandleFunc("POST /admin/counts/sync-runs", h.CreateSyncRun)
	mux.HandleFunc("GET /admin/counts/sync-runs/{sync_run_id}", h.GetSyncRun)
}
...
func (h *Handler) CreateSyncRun(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.service.Sync(r.Context(), app.SyncCountsInput{
		TenantID:       tenantID(r),
		ActorID:        actorID(r),
