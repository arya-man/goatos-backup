...
// TestDeriveShiftingImpactsRejectsAnythingButExactlyOneAnimal pins the boundary of the derivation.
//
// For zero animals there is nothing to describe. For two or more the server would have to invent
// the cohort split and the pregnancy/lactation/warm-up distribution across those cohorts -- business
// facts nobody entered. Both must fail rather than produce a plausible-looking guess.
func TestDeriveShiftingImpactsRejectsAnythingButExactlyOneAnimal(t *testing.T) {
	for name, goatIDs := range map[string][]string{
		"zero animals":  {},
		"nil animals":   nil,
		"two animals":   {destGoatID, destGoatIDB},
		"three animals": {destGoatID, destGoatIDB, "88888888-8888-4888-8888-888888888888"},
	} {
		t.Run(name, func(t *testing.T) {
			...
			// It must reject BEFORE touching the database: a request that can never be derived
			// should not cost a goat read.
...
func TestDeriveShiftingSourceReadsTheAnimalsCurrentPlacement(t *testing.T) {
	...
	parkID, shedID, err := svc.DeriveShiftingSource(context.Background(), destTenantID, []string{destGoatID})
	if err != nil {
...
func TestDeriveShiftingSourceDegradesInsteadOfInventing(t *testing.T) {
	t.Run("animal has no recorded placement", func(t *testing.T) {
		...
		parkID, shedID, err := NewService(repo).DeriveShiftingSource(
	...
	t.Run("not exactly one animal", func(t *testing.T) {
		repo := &fakeRepo{}
		for _, goatIDs := range [][]string{nil, {destGoatID, destGoatIDB}} {
			if _, _, err := NewService(repo).DeriveShiftingSource(
				context.Background(), destTenantID, goatIDs); !errors.Is(err, ErrImpactNotDerivable) {
				t.Fatalf("goatIDs=%v err=%v, want ErrImpactNotDerivable", goatIDs, err)
			}
		}
	})
}
...
func TestDeriveShiftingSourceFailsClosedWhenTheAnimalDoesNotResolve(t *testing.T) {
	svc := NewService(&fakeRepo{})
	if _, _, err := svc.DeriveShiftingSource(
