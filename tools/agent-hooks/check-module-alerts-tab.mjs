...
const PENDING_ALERTS_FEED = {
  ...
  vendors:
    "Procurement (module key vendors) shipped its vendor-register and feed-purchase screens before a module-scoped alert feed exists: there is no /vendors/alerts route or vendors alert producer over shared task/contact truth yet. Giving the bar an Alerts tab now would route operators to an empty screen. Unblocked when vendor onboarding or purchase exceptions produce a real feed. (Sales moved to its own module on 2026-09-05 and carries its own entry below; this reason no longer covers it.)",
  sales:
    "Sales became its own phone module on 2026-09-05 -- the ledger moved off the Procurement bar and took the selling half of the vendor register with it -- and it inherited the same gap: there is no /sales/alerts route and no sales alert producer over shared task/contact truth on any branch. Splitting the module did not create a feed, so an Alerts tab would route the sales desk to an empty screen, exactly as it would have inside Procurement. Unblocked when a sale exception (an overdue payment, a deal stuck in a status, a buyer lead going cold) produces a real feed.",
  leadership_tasks:
