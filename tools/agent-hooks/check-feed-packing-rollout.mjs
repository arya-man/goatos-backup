...
// -------------------------------------
// Migration 000149 collapsed feed_packing_completions to one row per pen-day and ADDED the pen-day
...
function selfTest() {
  const cases = [
    ...
    },
    {
      ...
      filename: "000150_feed_packing_drop_session_natural_key.sql",
      sql: `-- +goose Up\nDROP INDEX IF EXISTS ${COMPAT_INDEX};\n`,
      wantBlocked: true,
    },
  ];
...
function main() {
  ...
  if (findings.length) {
    ...
    console.error("\nSee backend/migrations/postgres/000149_feed_packing_pen_day_grain.sql and docs/decisions/feed-distribution-verification.md");
    process.exit(1);
  }
