...
out="$(bash -c '
  ...
  . "$1"
  step() { :; }
  # NOTE: no apostrophes in here - this whole block is single-quoted for bash -c.
  # The run-local-ci invariant is that every failure site goes through
  # record_failure, so the extracted function needs a stub for it.
  record_failure() { fail=1; }
