...
compose up -d --build api outbox-relay domain-event-consumer kernel-workers
...
for worker in vaccination-generator obligation-sweeper calendar-reminder-sweeper calendar-escalation-sweeper calendar-projector process-integrity-projector notification-dispatcher; do
...
wait_for "replayed outbox republished" \
  "test \"\$(psqlq \"select status from outbox_messages where event_id='$event_id'\")\" = published"
