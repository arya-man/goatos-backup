...
run_calendar_projectors() {
  ...
  run_go_cmd calendar-vaccination-projector -tenant-id "$tenant_id"

  # Future projection hook: when a dedicated calendar history projector lands,
  # the closeout will run it without changing every seed runbook again.
  run_go_cmd calendar-history-projection-recompute -tenant-id "$tenant_id"
}
