...
<!-- Coupling review 2026-08-14: pen capacity/cohort columns on shed_partitions are bounded catalog configuration used by the Counts/Sheds directory. They add no request-path tenant scan, no vaccination fixture mutation, no HRMS operator-capacity fan-out, and no derived projection workaround; seed/source validation remains byte-stable. -->
<!-- Coupling review 2026-08-26: migration 000210 makes procurement purpose part of the compiled protocol_rule_dimensions match index. This removes a broad generation scan for purpose-specific procurement plans; it adds no request-path tenant scan, projection workaround, source fixture mutation, HRMS seed contract, or seed-time recomputation. Existing dimensions stay equivalent through DEFAULT 'all'. -->
...
<!-- Coupling review 2026-08-15: sharing SchedulePathForGoat between seed import and runtime generation is a bounded generation-time policy call, not request-path compute. It changes no raw HRMS fixture bytes, SOP contracts, proof grain, source vaccination dates, or row counts. -->
<!-- Coupling review 2026-08-16: Flushing is a stage-vocabulary row for shifting destination-tag writes. Adding it to seed-vaccination-real and migration 000171 is not a new request-path read, fixture source field, SOP proof grain, source-date contract, or operator-capacity rule; HRMS fixture bytes and counts remain unchanged. -->
...
<!-- Coupling review 2026-08-29: manual-anchor suppression now covers manual_campaign seed rows as well as DOB/arrival/calendar seed rows. The lookup is batched per animal over same-family vaccine codes already in the active rule set; it adds no request-path tenant scan, fixture mutation, or seed-time fan-out beyond existing generation work. -->
<!-- Coupling review 2026-08-29: Blue Tongue adult W2/booster anchoring changes derived vaccination scheduling only. The generator uses the already-accepted dose-1/manual anchor and configured +21 day gap; the kid path is 16w plus 19w booster. It adds no request-path aggregation, tenant scan, projection workaround, HRMS fixture mutation, SOP/operator form field, or operator-capacity fan-out. -->
<!-- Coupling review 2026-09-01: vaccination stale-row cleanup adds tenant-scoped caches for active vaccination dose and rule-identity no-op lookups, plus a partial index for unlabelled active obligations. These are run-scoped generation repairs replacing repeated per-goat probes over OCI/STG; they do not introduce request-path aggregation, unbounded UI reads, seed fixture changes, or new leadership assistant surfaces. -->
