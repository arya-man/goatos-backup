...
reads): `weighing_fasting_tasks` (migration 000246, the ROUND: one park, one weigh
night, one removal operator, the dates and the midnight stamp) + the per-shed
evidence table `weighing_fasting_shed_proofs` (migration 000248, UNIQUE per
...
linked via `pc_care_tasks.gates_task_id` (migration 000247; partial unique
