...
- Backend status projection fix after judge review: whole-filter card summaries now preserve `verification_pending`/`proof_pending` before falling through to `completed`. A submitted partition with `open=0` is in review, not simply done/in progress.
- Android status projection fix after judge review: backend card summary status `verification_pending`/`proof_pending` renders as `In review` with the review chip. It does not reopen scan/finalize/submit controls.
- Backend submit membership fix after judge review: final `sop_submission_items` filtering for partitioned vaccination submit now uses the task's exact `vaccination_drive_assignment_members` through the task batch, derived shed scope, and submitted partition label. It no longer asks current `goat_shed_partitions` to decide whether an item belongs to this submission.
- Final post-review API proof: local backend returned five exact partition card summaries, all `verification_pending`, all `open=0`, with the expected vaccine groups: `ET+TT`, `PPR+FMD`, and `PPR+FMD+HS`.
- Final post-review phone screenshot evidence: `/tmp/goatos-phone-qa-implicit-submit/final-after-review-status-patch.png`; tap evidence stayed on the list at `/tmp/goatos-phone-qa-implicit-submit/final-after-review-card-tap.png`.
