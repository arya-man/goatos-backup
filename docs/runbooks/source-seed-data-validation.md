...
The shifting destination-profile snapshot columns added by
`000003_add_shifting_destination_profile_snapshot.sql` are schema-only runtime
audit fields. They are populated when a pending shifting event is authorized and
then revalidated when completion applies the movement. Source fixtures must not
seed those snapshot values directly; seed/reseed validation remains focused on
...
<!-- Coupling review 2026-09-01: vaccination generation now excludes procurement-blocked animals from scheduling and cancels stale open work for exited/procurement-excluded animals. This is runtime reconciliation only; raw HRMS/vaccination fixture bytes, hashes, counts, source dates, SOP proof contracts, and seed source validation semantics are unchanged. -->
<!-- Coupling review 2026-08-04: vaccination_drive_date_overrides requested/applied safe-date metadata is runtime override state for an existing admin command. Approved combo helper sharing changes only clinical scheduling code reuse. No source spreadsheet bytes, fixture hashes, row counts, HRMS rows, SOP contracts, protocol source rows, or seed closeout steps change. -->
...
<!-- Coupling review 2026-08-14: migrations 000160/000161 add Counts/Sheds pen-grain capacity and cohort config to shed_partitions. They do not alter source spreadsheet bytes, fixture hashes, HRMS rows, vaccination history dates, goat_shed_partitions placement parsing, SOP proof grain, protocol rows, or operator capacity. No source-audit validation rule changes. -->
<!-- Coupling review 2026-08-26: migration 000210 adds protocol_rule_dimensions.procurement_purpose as compiled execution-index metadata with DEFAULT 'all'. Publish fills it from already-authored vaccination rule DSL so generation can prefilter breeding/fattening procurement cohorts in SQL. No raw source spreadsheet bytes, fixture hashes, HRMS rows, SOP proof grain, source vaccination dates, or seed closeout steps change. -->
...
<!-- Coupling review 2026-08-15: seed import and runtime generation now share SchedulePathForGoat for kid/adult schedule-path selection. Raw source files, fixture hashes, HRMS rows, SOP contracts and vaccination source dates are unchanged; validation preserves DOB/stage/history evidence and must not reintroduce a seeder-local classifier. -->
<!-- Coupling review 2026-08-16: migration 000171 and seed-vaccination-real add Flushing to animal_stage_lookup with NULL age_band. This is stage-vocabulary configuration for shifting, not a raw HRMS/vaccination source field; fixture bytes, hashes, row counts, SOP proof grain, source vaccination dates, and source-audit validation stay unchanged. -->
...
<!-- Coupling review 2026-08-29: manual vaccination anchors now suppress same-family manual_campaign seed rows in addition to DOB/arrival/calendar seed rows. This changes derived replay behavior only: raw HRMS/vaccination fixture bytes, hashes, row counts, SOP contracts, accepted source history, and source-date validation semantics remain unchanged. -->
<!-- Coupling review 2026-08-29: accepted dose 1 or a manual dose-1 anchor drives dose 2/booster timing for every multi-dose vaccine course. DOB/age rules are only for finding the first due date when there is no accepted history. Once dose 1 is real, the booster follows the configured gap from that date, so source-date validation must not reinterpret it as a fresh DOB-only row. For Blue Tongue the gap is 21 days: 16w first dose, 19w booster. -->
<!-- Coupling review 2026-08-30: same-day compatible vaccination sessions now allow up to 3 vaccines per animal. This is a runtime planner/capacity policy and persisted config override; raw HRMS/vaccination source files, fixture row counts, SOP proof grain, and trusted source vaccination dates stay unchanged. The fixture manifest source_policy_sha256 was updated to pin the reviewed policy JSON. -->
<!-- Coupling review 2026-09-01: vaccination drive assignment rows are now unique at operator/date/shed/partition/vaccine_rule_ids grain, and operator daily animal caps are constrained to 1..200. This changes runtime scheduling storage and cap validation only; it does not alter raw source files, fixture hashes, row counts, SOP proof grain, trusted vaccination dates, or source-audit validation. -->
<!-- Coupling review 2026-09-01: feed experiment per-animal migration reads live goats to divide legacy pen-total kg into grams per animal for feeding. This is a feed configuration conversion, not a vaccination/HRMS source import change: no source files, fixture hashes, HRMS rows, SOP proof grain, trusted vaccination dates, roster capacity, or source-audit validation rules change. -->
