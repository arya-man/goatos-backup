...
| M1 | `calendarBusinessDateIn` IST fallback diverges from SQL. | backlog | No | Low-risk for clean dev seed; revisit with broader timezone hardening. |
| M2 | `birth_age`/`post_arrival` due calculations use UTC date arithmetic. | backlog | No | Clean dev seed uses explicit launch/business dates; revisit before multi-timezone production. |
