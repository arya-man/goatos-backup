...
<!-- Coupling review 2026-07-23: migration 000035 introduces vaccination_operator_shift_config and vaccination_operator_assignment_config tables for Phase-1 operator shift scheduling (shift_label/shift_start_minute/shift_end_minute) and default assignment rules (active_operators_per_day, default_operator_code). These are config-only in Phase 1 and do not change source fixture bytes, seed validation, or operator-roster contract schema verification; the validator and seed-roster-real accept and seed these fields when present in cpt-operator-roster.json. -->
