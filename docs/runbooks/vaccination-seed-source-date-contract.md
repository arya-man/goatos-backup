...
kid dose 2 stays 21 days after Blue Tongue kid dose 1. Single-dose vaccines
...
<!-- Coupling review 2026-08-14: shed_partitions.capacity and shed_partitions.animal_stage_id are pen-directory configuration fields for Counts/Sheds. They are not vaccination date anchors, HRMS capacity inputs, protocol source rows, SOP proof metadata, or goat_shed_partitions placement source. Fixture bytes, hashes, counts, and source-date validation remain unchanged. -->
<!-- Coupling review 2026-08-26: protocol_rule_dimensions.procurement_purpose is generated index metadata, not a source-date field. Existing rows backfill to 'all', and new rows derive from authored rule DSL during publish. This changes only generation prefilter selectivity for procurement breeding/fattening purpose plans; raw fixture bytes, HRMS roster rows, SOP contracts, trusted source dates, and validation semantics remain unchanged. -->
...
<!-- Coupling review 2026-08-15: seed-vaccination-real now calls the shared SchedulePathForGoat policy used by runtime generation. No fixture byte/hash/count/HRMS/SOP/source-date change is required; this removes the seeder-local shortcut and preserves reviewed DOB/stage/history as the only classification evidence. -->
<!-- Coupling review 2026-08-16: Flushing is now seeded as an active stage-catalog tag with NULL age_band, matching migration 000171 for existing tenants. This does not alter source vaccination dates, DOB/stage/history evidence, HRMS rows, SOP contracts, fixture hashes, or validation semantics. -->
...
<!-- Coupling review 2026-08-29: an explicit manual anchor is the first operational date for that vaccine family, so runtime generation suppresses same-family manual_campaign seed rows before the anchor just like DOB/arrival/calendar rows. Source dates and imported trusted history are unchanged; this does not alter fixture bytes, hashes, HRMS rows, SOP proof grain, or validation inputs. -->
<!-- Coupling review 2026-08-29: for any multi-dose vaccination course, accepted dose 1 or a manual dose-1 anchor is the medical anchor for dose 2/booster. Age/DOB rules only create the first due row for animals with no history; they must not delay or replace a booster once dose 1 exists. Example: Blue Tongue dose 1 accepted on 2026-08-12 schedules the +21 day booster on 2026-09-02, including adult/manual campaign animals; the kid DOB path is 16w + booster at 19w. -->
<!-- Coupling review 2026-08-30: max same-day vaccination count is 3, enforced by scheduler policy and vaccination_capacity_config override. This does not create or reinterpret source vaccination dates; source-date validation remains about imported/accepted history and booster anchoring from dose 1. Fixture bytes and row counts are unchanged; only the policy digest changes. -->
<!-- Coupling review 2026-09-01: vaccination drive assignment identity now includes the exact vaccine_rule_ids lane and operator daily animal cap is constrained to 1..200 in DB + app code. This prevents ET+TT/Z1+Z3 and other same-day lanes from collapsing into one operator card and keeps persisted caps aligned with the field rule. No raw HRMS/vaccination fixture bytes, source vaccination dates, SOP proof grain, roster rows, or validation inputs change. -->
<!-- Coupling review 2026-09-01: procurement-excluded goats are removed from vaccination generation reads, and stale open vaccination work for exited/procurement-excluded animals is canceled by the generator cleanup phase. Generic manual aliases such as `PPR`, `FMD`, and `HS` cannot auto-materialize through normal sweeper generation. This changes derived runtime obligations only; raw HRMS/vaccination fixture bytes, source dates, hashes, row counts, SOP contracts, and seed validation inputs are unchanged. -->
<!-- Coupling review 2026-09-01: feed experiment per-animal conversion reads goats only to count live residents of a feed pen before converting legacy kg totals. It does not create, suppress, or reinterpret any vaccination seed row, trusted source date, SOP proof field, HRMS roster row, or validation input. -->
