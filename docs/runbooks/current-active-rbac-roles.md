...
| `procurement_director` | Procurement Director; the whole Procurement vertical on admin-web (source entry incl. review, vendor register incl. finance, sales incl. record-sale) plus READ-ONLY Feed oversight (Feed Analytics + Feed SOP; **Feed Config is hidden** — no leaf, no page contract, no `feed_config.read`). Admin-web ONLY (no `app.bootstrap`). The narrowing is his own PAGE TICKS on /people, not code: the hand-coded `adminui/app/procurement_director_lens.go` was RETIRED on 2026-08-27 and the backfill wrote the same six screens onto his access rows, so changing what he sees is now an edit rather than a commit. Maintainer decisions 2026-08-21 (what he sees) and 2026-08-27 (how it is expressed) | `tenant` (both parks) | Hemant Singh (holds it ALONGSIDE `feed_director`) |
| `breeding_director` | Breeding Director; plans **hoof trimming and hair trimming** PC Care tasks (`pc_care.plan_trimming`, category-scoped — deworming / ticks removal stay CEO-planned) and reads the Preventive Care board. Holds NO `pc_care.execute` (a planner must not film the work they planned) and NO `pc_care.plan`. Per-person access row: `pc_trimming` at Configure. Maintainer decision 2026-09-04 | `tenant` (both parks) | Dinakar (holds it ALONGSIDE `growth_director` and `pc_director`) |
...
| Dinakar | `growth_director` (Weighing owner), `operator` | `counts_approver`, `pc_director`, `growth_director`, `breeding_director`, `operator` | Same approval authority, plus Preventive Care authority; keeps `growth_director` so Weighing still has an accountable director. `breeding_director` (2026-09-04) is his designation and the desk that plans hoof / hair trimming |
| Hemant Singh | `feed_director` (manual STG seed 2026-08-10) | `procurement_director` | Procurement Director on admin-web (maintainer decision 2026-08-21). Keeps `feed_director`: his phone access, feed-proof notification routing and feed-chain write authority ride on it; `procurement_director` layers the web-only Procurement + Feed workspace on top |
