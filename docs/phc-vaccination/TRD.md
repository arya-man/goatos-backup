...
Shed/cohort `animal_stage` is the **default** anchor, but eligibility combines **age + sex + lifecycle_status + health_status + shed/cohort animal_stage + explicit overrides**. Exceptions the engine must honor: ICU / quarantine (defer, don't fire), pregnant / lactating (different rule or skip), sick / under_treatment (defer), and **individual catch-up** (a goat vaccinated standalone outside its shed's stage). Encode the predicate in `protocol_rules.eligibility_json`; encode per-goat exceptions as an explicit override (an obligation can be `deferred` with `block_reason`, or generated individually) — never silently inherit only the shed value.
...
- **`animal_stage_lookup`** (drives vaccination eligibility): `stage_id PK, tenant_id, stage_code CHECK('K0','K1','K2','K3','F2','mother','pregnant','milking','icu'), label, min_age_days, max_age_days, min_weight_kg, max_weight_kg, sort_order`, UNIQUE `(tenant_id, stage_code)`. **Eligibility uses THIS.**
...
Age/weight bands that decide stage live on `animal_stage_lookup` as **config** — the mock's hardcoded `SHIFT_THRESH` (K1=7/K2=45) must be removed and read from here. **Reconcile K2 = 42 vs 45** with PHC.
...
- **Individual catch-up** (`is_individual`): one `consume` movement per **accepted** completion (no prior reserve).
