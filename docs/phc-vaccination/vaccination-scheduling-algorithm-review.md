...
**Purpose:** Vaccination drives cannot be created by “everyone in the shed is due something.” Each goat/sheep must follow species-specific schedules, intake path, mother status, age windows, cross-vaccine gaps, pregnancy blocks, and defer states. Drives are a **second layer** that groups only **medically compatible, window-valid** obligations.
...
2. **Rule-matrix lookup** — species + intake path + mother status + age → which dose rows apply.
...
2. Intake / life path?
   A. Farm-born kid           → full kid schedule (mother-vaccinated vs not)
...
**Output of classification:** `schedule_profile` = `{species, path, mother_vaccinated?, age_weeks, reproductive_phase, defer_reason?}`
...
| `mother_vaccinated_branch` | which timing row applies |
...
**Mother vaccinated vs not** changes timing:

| Vaccine | Mother vaccinated | Mother NOT vaccinated | Revaccination |
| --- | --- | --- | --- |
| ET+TT | 4 wk + 7 wk | 1 wk + 5 wk | 6 months |
| PPR | 16 wk | 4 wk | 3 years |
| Blue Tongue (sheep) | 16 wk + 20 wk | 4 wk + 7 wk | 1 year |
| Goat Pox | 16 wk | 4 wk | 1 year |
| Sheep Pox | 12 wk | 4 wk | 1 year |
| FMD | 12 wk | 4 wk | 9 months |
| HS | 12 wk | 4 wk | 1 year |
...
1. **Load goat facts:** species, approx_dob, entry_date, origin_type (born/procured), mother_vaccinated (if known), current shed → animal_stage, sex, lifecycle, health, reproductive_status, warming_entry_date.
...
3. **Filter matrix rows** where eligibility matches:
   ...
   - mother_vaccinated branch (kids only)
...
## 10. Mother vaccinated signal

Kid schedule branches on mother status. Algorithm needs:

- **Input:** `mother_vaccinated: true | false | unknown`
- **unknown** → conservative path (treat as mother NOT vaccinated OR hold for PHC review—**needs product decision**)
- **Source:** dam linkage if kid born on farm; procurement record if imported with dam history
...
| Mother-vaccinated branch | **Gap** — needs dam/history signal in eligibility |
...
2. **Mother vaccinated** (kids): boolean or unknown + provenance
3. **Warming entry timestamp** (for 7-day hold)
4. **Trusted vaccination history:** vaccine, dose, administered_at, source, approval status
5. **Published protocol versions** per species/path with full matrix rows
6. **Vaccine metadata:** live/killed class, dose_ml, combo groups, priority
7. **Inventory/stock** (for drive feasibility, not eligibility)
...
1. **Mother vaccinated = unknown** → use early schedule (not vaccinated) or PHC hold?
2. **Kid cutoff:** strict through 16 weeks only, or also 14 weeks as separate stage gate?
3. **Live→live gap:** confirm **4 weeks** (source doc) vs any ops practice of 2 weeks.
4. **Source vaccination at procurement + warming hold:** if vaccinated at source Monday and arrives warming Tuesday, is next farm dose earliest day 8 after warming entry, or 7 days after source dose?
5. **Post-delivery catch-up:** all skipped pregnancy doses in 2 weeks, or priority order if multiple due?
6. **Sheep Blue Tongue on adult path:** booster timing relative to step 2 pox bundle.
7. **Untrusted procurement vaccination mentions:** catch-up full intake protocol or trust with review queue?

---

## 16. One-line summary for reviewers

> We are building a **two-layer vaccination orchestrator**: first compute **per-goat medically correct dose obligations** using species, intake path, mother status, age matrix, gaps, pregnancy, and defer rules; then **only batch compatible obligations into shed drives** using a conflict graph—never schedule a shed drive from headcount alone.
