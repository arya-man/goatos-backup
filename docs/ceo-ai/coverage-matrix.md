...
| person_module_access_sales_mobile_backfill | excluded (migration ledger) | Rows written by migration `000269` so its Down path removes only the mobile Sales ticks it created. Identical shape and reason to the vendors ledger above: two-column bookkeeping with no leadership fact. |
