...
| path:/economics/overview (GET /economics/overview) | api | The Sales → Economics page in one read (leadership-only, SalesEconomicsRead): per-animal daily feed cost (priced feed direction) beside measured daily gain (weighing pairs), cost per kg of gain, realized price per kg from closed deals, break-even weight bands, and per-animal apportioned sale revenue via the identity-owned goat_sale_allocations mapping. Whole-filter pulse aggregates ride the same response as the capped tables, per the operational read-model contract. Served by the read-only `backend/internal/economics` module (Growth Director shape); every rupee figure is a disclosed estimate (`estimate: true`). Leadership questions like "what does a kg of gain cost us" and "which animals lose money per day" route here. Cube/`ceo_ai` mapping is future work, same as the sales ledger's. New module plumbing is not an independent read surface: `func:NewService`, `func:GetBusinessEconomics`, `func:NewHandler`, `func:NewRepository`, `func:Register`, `func:ListParks`. |
