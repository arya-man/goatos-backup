...
| GET /counts/sheds (func:GetShedDirectory, func:ShedDirectory) | api | The shed CONFIGURATION directory at OPERATIONAL LOCATION grain — every pen, and every shed that has no pens, with the cohort tag configured for it and the head count it is meant to hold. It is deliberately NOT a census: `GET /counts/breakdown` above answers "how many animals are standing where", this answers "what did we build and what is it for", and a pen holding zero animals still appears. Leadership-relevant because capacity headroom is a planning fact ("how many head can CBE hold, and how much of that is already committed"), and the read API is the coverage path: the rows are a bounded catalog (~120 pens) returned whole with `total_rows` independent of paging, so the assistant reads the same numbers the screen shows. No Cube metric or `ceo_ai.*` view is added — inventing a capacity KPI nobody has asked for would be the wrong direction; if leadership later asks a stocking-ratio question, that becomes a real aggregate row against this catalog joined to the census, not a second copy of it. |
| func:PivotShedDirectory (counts/domain) | EXCLUDED | Pure presentation pivot for the row above: it turns the flat (park, shed, pen) rows the query already returned into one row per operational-location LABEL with a cell per park, so the two parks' same-named locations read side by side. It derives no fact, reads no data, and adds no API, metric, view or tool — every cell keeps its own park's `shed_id`, and the tag and capacity it arranges are already covered by `GET /counts/sheds`. |
