...
| func:ListCampaignsForOperator, func:ListScopeRoster, func:ListScopeRosterForOperator | EXCLUDED | Operator-only execution read helpers for mobile shed buckets and individual rosters. They exist to keep `/app/weighing/campaigns` and `/app/weighing/campaigns/{campaign_id}/sheds/{campaign_shed_id}/roster` scoped before pagination/row return. Leadership assistant coverage remains `GET /weighing/campaigns`; no MCP/Toolbox, Cube, or `ceo_ai` SQL fallback surface is added. |
