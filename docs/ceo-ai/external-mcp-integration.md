...
| Health work/cases | `get_health_today`, `get_health_work_items` | `GET /app/health/work-items` | Treatment-session grain. Use `get_health_today` for broad questions so adult and kids work are both included. Open sick work is not a death event unless health state says so. |
