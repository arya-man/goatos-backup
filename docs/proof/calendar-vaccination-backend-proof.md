...
9. Read models: Calendar list/detail/history are source-backed projection reads with action/audit/notification/snooze history and tenant/park/shed authorization scope; they do not reconstruct truth in frontend state.
...
11. Operations: projector and sweeper have bounded `-limit`; reminder sweeper requires an explicit tenant and persists each event in its own transaction; outbox rows feed the existing relay; audit/history rows carry tenant, event id, action, channel, trace, status, and idempotency metadata.
...
13. Seed/E2E data: seed covers CBE/CPT, PHC/Inventory/Admin Data Ops, due/overdue/future/in-progress/proof/verification/rework/deferred/blocked/snoozed/escalated statuses, negative system exclusion, scope-negative cases, and reminder history.
