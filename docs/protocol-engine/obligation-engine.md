...
```
**Lifecycle phases** (e.g. 0–12mo primary+booster, >12mo `repeat:yearly after_age 12mo`) are expressed as multiple `schedule` rows, not one booster flag. The whole `rule_dsl` is one immutable published `protocol_version`; a schedule change = a new version.
