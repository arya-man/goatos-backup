...
**Rule fields:** module/`category` (vaccination, feed_direction, deworming, sanitation, …) · multi-factor eligibility (age, animal_stage, sex, breed, lifecycle, health_status, shed/cohort/park, reproductive[exclude pregnant/lactating], defer_states[ICU/quarantine/sick], individual_override) · **`schedule[]` — the Schedule Builder: an array of dose/phase rows** (dose_code · trigger_type[birth_age/post_arrival/calendar/after_previous_completion/manual_campaign] · offset_days · due_window_days · min_gap_days · repeat[none/every_n_days/yearly/until_age/after_age] · repeat_until_after_age · catch_up · per-dose sop_version + proof_policy) — NOT a single trigger-day + booster flag · `missed_dose_policy` (immediate/next_cycle/phc_approval/defer) · `next_due_basis: last_accepted_completion_else_dob` · park override (scope_type/scope_id) · `effective_from`/`effective_to`. **Source/review metadata — nested `source` object on `rule_dsl`** (canonical shape; matches Config screen JSON): `source:{ source_system` (vaccinations_db/phc/vet/manual_admin) `· source_ref · imported_at · reviewed_by · review_status` (extracted→reviewed→approved) `· approved_by · approved_at }`.
...
**Publish gate + dev policy (source-backed):** a version publishes **only when `review_status='approved'`** and source-backed. Values that come from a **real source (Vaccinations DB / PHC / vet-approved)** and are marked `approved` are **real config in `goatos-dev` and publishable there** — the **dev-real path**. **Unsourced / `manual_admin` / `extracted`** rows stay `status='draft'` and carry a **`not source-backed`** warning — they cannot be published and generate no production work. **Never hand-invent** PPR/FMD/ET schedule values; real values arrive via this config UI or a reviewed extract from the live Vaccination DB.
...
`rule_id PK · tenant_id · protocol_version_id→protocol_versions · dose_code text (primary/booster_1/booster_2/annual/catch_up/…) · sequence int · trigger_type text CHECK(birth_age/post_arrival/calendar/after_previous_completion/manual_campaign) · offset_days int · due_window_days int · min_gap_days int · repeat text CHECK(none/every_n_days/yearly/until_age/after_age) · repeat_until_after_age text · catch_up text CHECK(immediate/next_cycle/phc_approval/defer) · eligibility_json jsonb · sop_version_id uuid NULL (per-dose override) · proof_policy jsonb · withdrawal_days int NULL · sort_order int`.
...
{ category, scope:{type,id},
  eligibility:{ age_band, animal_stage, sex, breed, lifecycle_status, health_status,
                ...
                defer_states:[ICU,quarantine,sick], individual_override:bool },
  missed_dose_policy: immediate | next_cycle | phc_approval | defer,
  next_due_basis: last_accepted_completion_else_dob,
  schedule: [                                 // ARRAY — multi-dose / lifecycle phases
    { dose_code, sequence, trigger_type, offset_days, due_window_days, min_gap_days,
      repeat, repeat_until_after_age, catch_up, sop_version, proof_policy:[…] }, … ],
  escalation,
  source:{ source_system, source_ref, imported_at, reviewed_by,        // provenance + approval gate
           review_status, approved_by, approved_at } }
```
**Lifecycle phases** (e.g. 0–12mo primary+booster, >12mo `repeat:yearly after_age 12mo`) are expressed as multiple `schedule` rows, not one booster flag. **Per-dose SOP + proof_policy** link each row to its execution form. The whole `rule_dsl` is one immutable published `protocol_version`; a schedule change = a new version.
...
The unit of execution is the **batch**, not the individual obligation: **many due obligations → grouped into one `obligation_batch` → one `sop_task`.** The per-obligation `sop_task_id` (kept for the rare individual catch-up) and the batch's `sop_task_id` reference the same task for a group drive.
...
Assignment uses `workforce_member_capabilities` (e.g. `vaccination.execute` scoped to the shed's park). Verification uses `proof.verify`. Individual catch-up (`is_individual`) skips the batch and spawns its own `sop_task`. No new task engine.
...
- **Individual catch-up (`is_individual`):** `consume` 1 dose per **accepted completion** (no prior reserve).
...
| Flip due + dispatch near-term | `obligation-sweeper` (Cloud Run Job ← **Cloud Scheduler** cron) | scan today's window via `(tenant_id, status, due_at)` idx → enqueue **Cloud Tasks** for near-term only |
...
1. **No per-goat `sop_task` for group work.** Group execution = ONE `sop_task` per `obligation_batch` (shed drive / feed session). Per-goat tasks only for genuine individual catch-up. 1M goats must never become ~1M human tasks.
