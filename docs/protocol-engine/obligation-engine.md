...
3. **Due scans use the `(tenant_id, status, due_at)` index** and touch only the current indexed due window — never a full-table or full-herd scan. Sweeper pages through results, bounded batch size, resumable. If `obligation_instances` is later partitioned, the same query must also prove partition pruning.
4. **Archive/partition policy for growth.** `obligation_status_events` is currently RANGE-partitioned by date. `obligation_instances`, `obligation_batches`, and `vaccination_completions` are currently indexed, non-partitioned tables; their measured partition/retention decision remains open under the [One-Million PostgreSQL Readiness ADR](../decisions/one-million-postgres-readiness.md). There is **no** `vaccination_schedule` table — per-animal due state lives in `obligation_instances`. Terminal history must follow an explicit archive/retention policy so the hot set stays bounded, and Control Tower never aggregates over cold history live.
