...
to Preventive Care (PC) / Vaccination UI and foundation review is satisfied by the local Goal 1
...
shed's absolute per-shed kg, which GoatOS executes on the normal Feed path.
...
4. Warmup, pregnancy, K0/K1, breed aliases, native experiment absolute-kg
   allocations, and versioned session-slot/feed-set policy are explicitly
