...
| Animal entry paths | Closed for V1 demo | Manual animal API path and procurement accepted-intake matrix proof pass in `NUANCE-RULES-20260701-V1-MATRIX-R2`; Herd Register visual route is captured. | Full browser CSV/import negative matrix is production hardening. |
...
| SOP field types | Closed for V1 demo | Authoring smoke covers text, number, yes/no, select, multiselect, animal scan/RFID, shed picker, vaccine batch, medicine picker, photo proof, video proof. | Additional usability polish can continue after demo. |
