...
resource "google_cloud_run_v2_job" "vaccination_schedule_projector" {
  name                = "goatos-stg-vaccination-schedule-projector"
  location            = var.region
  deletion_protection = false
  labels              = local.labels
  ...
  template {
    ...
    template {
      service_account = google_service_account.runtime["vaccination_schedule_projector"].email
      timeout         = "900s"
      max_retries     = 0

      containers {
        image   = local.backend_image
        command = ["/app/bin/vaccination-schedule-projection-recompute"]
        args    = ["-timeout=10m", "-dirty-limit=500", "-months-back=1", "-months-ahead=13"]
