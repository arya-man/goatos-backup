...
| Exhausted notification follow-up | Notification retry, terminal `exhausted` state, dispatch counter, audit row, `notification.exhausted` outbox event, and dev Terraform webhook secret wiring exist. | Add the full exhausted-alert operating workflow: fallback notification or escalation handler, operator queue, exported metric, owner assignment, reroute policy, closure evidence, and recipient proof. |
| Notification evidence idempotency backstop | Exhausted notification outbox writes are independently idempotent, and the audit row is emitted inside the same guarded `sending` -> `exhausted` transition; replay of the same exhausted claim is covered by integration test. | If a future re-exhaust, re-notify, or manual repair path bypasses that guarded transition, add an audit-level idempotency policy that works with monthly partitioning. |
...
| Production Preventive Care (PC) roster and rule load | The local slice proves the engine with source-derived dev baseline data. | Load and approve the real Preventive Care (PC) schedule/roster/rule set before production use. |
