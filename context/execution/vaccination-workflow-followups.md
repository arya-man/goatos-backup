...
| Missed-dose response ownership | Missed work is recorded and visible in the vaccination views; the durable `obligation.missed` event is consumed and drives the existing Calendar escalation sweep. | Define the business owner, follow-up wording, escalation timing, closure rule, and how repeated missed work is handled. |
