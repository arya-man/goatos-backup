...
**Status:** IMPLEMENTED 2026-07-13 on branch `fix/ledger-closure-20260712` (not yet committed).
The package-scoped container + migrated-template + per-test clone model is live in
`backend/internal/platform/pgtest` (`pgtest.go`, `lifecycle.go`); all 31 `StartPostgres` packages
were wired with a `TestMain` calling `pgtest.RunMain(m)` (the two E2E packages call
`pgtest.Shutdown()` from their existing `TestMain`). A static, Docker-free guard
(`TestLifecycleContractGuard`) fails the build if any `StartPostgres` caller package lacks that
teardown, so the "MUST wire TestMain" contract is enforced, not just documented.

Verified green (Docker): harness self-tests (isolation, migrated-schema present, cross-connection
committed reads, template rejects direct connections, and a leaked-connection **bounded-cleanup**
regression), the calendar hotspot (28 calls: ~90s → 7s), three concurrency-heavy packages run
concurrently, the HRMS E2E suite, and the cmd integration packages — all with zero orphaned
containers of this harness's naming and zero leaked volumes; peak concurrent containers observed =
12 under the existing `go test ./...` package-process model. `go vet ./...` is clean. An escape
hatch (`pgtest.StartDedicatedPostgres`) plus a `GOATOS_PGTEST_DEDICATED=1` toggle route back to the
pre-optimization one-container-per-test model.

Two pre-existing branch failures — `vaccinationexecution` ×3 ("read model projection unavailable")
and the kernel-story E2E suite's `TestKernelStoryY_CrossVaccineGap` (vaccine due-date floor,
`got=2026-07-19 want=2026-07-23`) — reproduce identically under the dedicated toggle and are NOT
caused by this change. The full kernel-story E2E run is therefore 40/41 on this branch; that one
failure is a branch defect outside this harness's scope. NOTE: the kernel-story `TestMain` rewrites
`backend/tests/e2e/report/index.html` on every run, so do not run a focused subset of that suite
against the committed report — it overwrites the 41/41 certification artifact with a partial one.

The original design write-up below is retained as the rationale of record.
