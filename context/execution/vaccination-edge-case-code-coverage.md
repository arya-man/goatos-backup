...
| New-goat generation | `goat.created` drives vaccination generation through the same idempotent generation path. Trusted HF/imported vaccination history suppresses duplicates. | `backend/internal/vaccination/app/generation_handler.go`, `backend/internal/vaccination/app/generation.go`, `backend/internal/vaccination/adapters/postgres/generation_integration_test.go` |
...
| Shed drive grouping | Sweeper groups due obligations into scope/shed batches and can create SOP tasks. | `backend/internal/obligation/app/sweeper.go`, `backend/cmd/obligation-sweeper/main.go` |
...
| Due goats are grouped shed-wise. | True through obligation sweeper/batching. |
