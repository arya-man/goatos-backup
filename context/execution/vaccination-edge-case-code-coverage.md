...
| Shed/park drive clubbing | Sweeper groups generated per-animal obligations into compatible shed/park drives. Exact per-animal due dates are not drive boundaries: nearby compatible animals must be delayed within the authored safe window / one-time batching hold to maximize drive output. Micro-drives are valid only when no compatible work can be safely clubbed before the earliest selected animal's last safe date. Wide-window sweeps use explicit `asOf` for planner math and `dueBefore` only for candidate loading. | `backend/internal/obligation/app/sweeper.go`, `backend/internal/obligation/app/preflight.go`, `backend/internal/obligation/app/drive_planner.go`, `backend/tests/e2e/story_ak_drive_clubbing_test.go`, `backend/cmd/obligation-sweeper/main.go` |
| Clubbed drive read-model date | Batched drive rows display, sort, and classify by `obligation_batches.planned_date`; animal `due_at` remains member obligation truth and is only the fallback for unbatched rows. | `backend/internal/calendar/adapters/postgres/canonical_read.go`, `backend/internal/vaccinationexecution/adapters/postgres/repository.go`, `backend/internal/processintegrity/adapters/postgres/repository.go`, `backend/tests/e2e/story_ak_drive_clubbing_test.go` |
...
| Due goats are clubbed into output-maximizing drives inside buffer. | True through generation -> obligation sweeper/batching -> Calendar, Vaccination Execution, and Process Integrity projections; guarded by `make vaccination-drive-clubbing-guard`. |
