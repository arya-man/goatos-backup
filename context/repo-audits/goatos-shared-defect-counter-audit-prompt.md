...
Do not push feature/code changes to main by default. Do not casually push commits
that trigger GitHub Actions. Do not use GitHub Actions as the default proof path.

Canonical ledger and audit-prompt docs are different: they are the Claude/Codex
shared sync surface and must be pushed to `main` so both agents see the same
truth. Ledger/audit-doc commits must be docs-only, use `[skip ci]`, and must not
carry feature code, generated clients, migrations, build artifacts, or unrelated
dirty files.
...
MAIN SYNC RULE WHILE GITHUB ACTIONS IS PAUSED
For feature/code commits:
- Do not push to main unless the user explicitly authorizes remote sync and
  CI-skip behavior is confirmed.
- If remote sync is authorized, confirm skip-CI behavior before pushing.
...
For canonical ledger/audit-doc commits:
- Push to `main` after each material ledger/audit-doc commit.
- Fetch/reopen `origin/main` first, re-apply only the ledger/audit-doc change on
  top, commit with `[skip ci]`, and push that docs-only commit to `main`.
- Never push feature code as part of a ledger sync.
- If the push is rejected, fetch, reopen the remote ledger/prompt from disk,
  preserve peer rows, re-apply only your section/append row, recommit, and push
  again.
- Record MAIN_PUSHED with commit SHA in Sync Notes. If a network/auth outage
  blocks the push, record PUSH_BLOCKED with the exact reason and retry plan.
...
6. Push the ledger/audit-doc commit to `main` under the Main Sync Rule above.
