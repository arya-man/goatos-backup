...
- Branch + full HEAD: `fix/ledger-closure-20260712` @ `<INTEGRATED_SHA>` (filled at commit; verified on `origin/main`).
...
Layman explanation: The repository shows a clean promotion PR, but both current checks died before running even one test.  
Evidence: Read-only `gh` for authenticated account `ravimesha` and `vgoats/goatos` shows open PR #3 (`main -> stg`) at exact HEAD with empty `statusCheckRollup`; current push and pull-request workflow runs are `startup_failure`, zero jobs. Several immediately preceding commits show the same state. Local `actionlint` passed, so the root external/config cause is not proven.  
