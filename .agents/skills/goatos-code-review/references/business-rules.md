...
- Vaccination seed/source-date contract: `docs/runbooks/vaccination-seed-source-date-contract.md`
...
- **Seed source dates are history anchors, not due work:** when reviewing seed,
  ...
  suppressed, and future obligations are generated by the kernel from that base.
  Seed/import code must not duplicate kid/adult path logic; it must call the live
  vaccination schedule-path policy so stale `origin=birth` or `K1/K2` source
  tags cannot resurrect past kid-course work. A raw sheet vaccination cell must
  never be mapped as `_kid_` history before path selection to prove its own
  kid/adult path; classify from independent DOB/stage/entry/accepted-history
  evidence first, then map and persist the source date under that path.
